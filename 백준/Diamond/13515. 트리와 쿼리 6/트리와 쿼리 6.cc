...
void updateFrag(int Fnum) {
    Fragsize[0][Fnum] = Fragsize[1][Fnum] = 0;
    for (int x : Frag[Fnum]) {
        ...
        Fragsize[col[x]][Fnum] += nodetoFragParent[x];
    }
}

int dfsFrag(int Fnum, bool c) {
    int ret = Fragsize[c][Fnum];
    for (int CF : FragchildFrag[Fnum]) {
        if (nodetoFragParent[FragParent[CF]] && col[FragParent[CF]] == c) ret += dfsFrag(CF, c);
    }
    return ret;
}
...
int dfsnode(int x, bool c) { //
    if (col[x] != c) return 0;
    int ret = 1;
    for (int nx : childinFrag[x]) ret += dfsnode(nx, c);
    for (int CF : nodechildFrag[x]) ret += dfsFrag(CF, c);
    return ret;
}
...
void solve(int tc) {
    ...
    Fragsize[0].resize(Frag.size()); Fragsize[1].resize(Frag.size());
    FragchildFrag.resize(Frag.size());

    for (int i = 0; i < Frag.size(); i++) {
        reverse(Frag[i].begin(), Frag[i].end());
        Fragsize[0][i] = Frag[i].size();
        Fragsize[1][i] = 0;
        for (int x : Frag[i]) {
            col[x] = 0; //black
            Fragnum[x] = i;
