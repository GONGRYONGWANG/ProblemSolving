...
ll DP[2000][(1 << 12)];
ll dp(int x, int hist) {
    if (x == N) {
        for (int k = 0; k < K - 1; k++) {
            if (!(hist & (1 << k)) && (hist & (1 << (k + 1)))) return -INF;
        }
        return 0;
    }
    if (DP[x][hist] != INF) return DP[x][hist];

    DP[x][hist] = dp(x + 1, hist);

    for (int k = 0; k < K; k++) {
        if (hist & (1 << k)) continue;
        DP[x][hist] = max(DP[x][hist], dp(x + 1, hist + (1 << k)) - arr[x].first * d[hist][k] + (arr[x].first + arr[x].second) * (K - k));
    }
