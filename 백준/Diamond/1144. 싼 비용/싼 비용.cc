...
int board[9][9];
unordered_map<deque<int>, int, DequeHasher> DP;

deque<int> norm(deque<int> hist, int a = -1, int b=-1) {
    unordered_map<int,int> m;
    for (int i = 0; i < hist.size(); i++) {
        if (hist[i] == 0) continue;
        if (hist[i] == b) hist[i] = a;
        if (!m.count(hist[i])) {
            m[hist[i]] = m.size() + 1;
        }
        hist[i] = m[hist[i]];
...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            unordered_map<deque<int>, int, DequeHasher> nxt;
            for (auto x : DP) {
                ...
                if (hist.front() != 0) {
                    ...
                    for (int i = 1; i < hist.size(); i++) {
                        if (hist[i] == hist.front()) fc = true;
                    }
                    if (fc) {
                        nxthist = hist;
                        nxthist.pop_front(); nxthist.push_back(0);
                        nxthist = norm(nxthist);
                        if (!nxt.count(nxthist)) nxt[nxthist] = inf;
                        nxt[nxthist] = min(nxt[nxthist], val);
                    }
                    
                    if (j != 0 && hist.back() != 0) {
                        nxthist = hist;
                        nxthist.pop_front(); nxthist.push_back(hist.front());
                        nxthist = norm(nxthist, hist.front(), hist.back());
                        if (!nxt.count(nxthist)) nxt[nxthist] = inf;
                        nxt[nxthist] = min(nxt[nxthist], val + board[i][j]);
                    }
                    else {
                        nxthist = hist;
                        nxthist.pop_front(); nxthist.push_back(hist.front());
                    ...
                    if (j != 0 && hist.back() != 0) {
                        nxthist = hist;
                        nxthist.pop_front(); nxthist.push_back(hist.back());
                        nxthist = norm(nxthist);
                        if (!nxt.count(nxthist)) nxt[nxthist] = inf;
                        nxt[nxthist] = min(nxt[nxthist], val + board[i][j]);
                    }
                        ...
                        nxthist = hist;
                        nxthist.pop_front(); nxthist.push_back(10);
                        nxthist = norm(nxthist);
                        if (!nxt.count(nxthist)) nxt[nxthist] = inf;
                        nxt[nxthist] = min(nxt[nxthist], val + board[i][j]);
                    }
