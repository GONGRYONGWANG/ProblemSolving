...
void solve(int tc) {
    ...
    for (int g = 1; g <= M; g++) {
        for (int i = 0; i < G[g].size(); i++) {
            graph.add_clause_then(G[g][i], idx + i);
            graph.add_clause_then(G[g][i], idx + G[g].size() + i);
        }
        for (int i = 0; i + 1 < G[g].size(); i++) {
            graph.add_clause_then(G[g][i], idx + i + 1, true, false);
            graph.add_clause_then(G[g][i + 1], idx + G[g].size() + i, true, false);
            graph.add_clause_then(idx + i + 1, idx + i);
            graph.add_clause_then(idx + G[g].size() + i, idx + G[g].size() + i + 1);
        }
        idx += G[g].size() * 2;
    }

    cout << (graph.satisfiable() ? "TAK" : "NIE");
