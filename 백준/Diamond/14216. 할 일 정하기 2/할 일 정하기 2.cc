...
struct Hungarian {
    ...
    vector<vector<int>> cost;  // 비용 행렬
    vector<int> u, v, p, way;  // 레이블 및 경로 추적
    vector<int> matchResult;   // 최종 매칭 결과 (작업자 -> 작업)

    Hungarian(int n) : n(n), cost(n, vector<int>(n, 0)), u(n + 1, 0), v(n + 1, 0), p(n + 1, 0), way(n + 1, 0), matchResult(n, -1) {}
