...
int parent[100001];
unordered_map<int, pll> branch[100001];
unordered_map<int, pll> dist[100001]; // branch, dist
pll total[100001];
...
void dnc(int x, int prvcent = -1) {
    ...
    while (!q.empty()) {
        ...
        for (pll& e : E[cur]) {
            ...
            dist[cent][nx] = { dist[cent][cur].first, dist[cent][cur].second + e.second };
            q.push({ nx,cur });
        }
    }
    ...
    while (Q--) {
        ...
        if (t == 1) {
            ...
            while (cent != -1) {
                ...
                total[cent].first += 1;
                total[cent].second += dist[cent][x].second;

                branch[cent][_branch].first += 1;
                branch[cent][_branch].second += dist[cent][x].second;

                cent = parent[cent];
            }
        }
        else {
            ll ans = total[x].second;
            int cent = parent[x];
            while (cent != -1) {
                int _branch = dist[cent][x].first;

                ans += total[cent].second - branch[cent][_branch].second;

                ans += dist[cent][x].second * (total[cent].first - branch[cent][_branch].first);
