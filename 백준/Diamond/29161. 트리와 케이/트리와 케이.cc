...
int parent[200001];
unordered_map<int, unordered_map<ll, ll>> branch[200001]; // {distance, xor}
unordered_map<int, pll> dist[200001]; // {branch , distance}
unordered_map<ll, ll> total[200001]; // distance,xor
...
void dnc(int x, int prvcent = -1) {
    ...
    for (pll& e : E[cent]) {
        ...
        q.push({ nx,cent });
        dist[cent][nx] = { nx,e.second };
        branch[cent][nx][e.second] ^= nx;
        total[cent][e.second] ^= nx;
    }
    ...
    while (!q.empty()) {
        ...
        for (pll& e : E[cur]) {
            ...
            int _branch = dist[cent][cur].first;
            ll _distance = dist[cent][cur].second + e.second;
            dist[cent][nx] = { _branch, _distance };

            branch[cent][_branch][_distance] ^= nx;
            total[cent][_distance] ^= nx;
...
void solve() {
    ...
    while (M--) {
        ...
        while (cent != -1) {
            ...
            ll _distance =  d - dist[cent][x].second;
            if (_distance < 0) {
                cent = parent[cent];
                continue;
            }
            int _branch = dist[cent][x].first;

            if (total[cent].count(_distance))ans ^= total[cent][_distance];
            if (branch[cent][_branch].count(_distance))ans ^= branch[cent][_branch][_distance];
