...
void solve() {
    ...
    cin >> N;
    MCMF graph(2 * N + 4);
    int src = 2 * N; int sink = 2 * N + 2;
    int src2 = 2 * N + 1; int sink2 = 2 * N + 3;
    graph.addEdge(src, src2, N, 0);
    graph.addEdge(sink2, sink, N, 0);
    for (int i = 0; i < N; i++) {
        graph.addEdge(src2, i, 1, 0);
        graph.addEdge(i + N, sink2, 1, 0);
    }
