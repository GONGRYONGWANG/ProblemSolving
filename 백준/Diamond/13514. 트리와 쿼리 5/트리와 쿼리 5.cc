...
void dfs(int x , int cent, int p, int depth) {
    dist[cent][x] = depth;
    for (int& nx : E[x]) {
        if (nx == p || visited[nx]) continue;
        dfs(nx, cent, x, depth + 1);
    }
}

void dnc(int x, int prvcent = 0) {
    int cent = getcent(x, -1, getsz(x));
    parent[cent] = prvcent;
    dfs(cent , cent, -1, 0);
    visited[cent] = true;
    whitedist[cent].insert(inf);
    for (int& nx : E[cent]) {
...
void solve() {
    ...
    while (M--) {
        ...
        if (t == 1) {
            ...
            int cur = x;
            while (cur) {
                if (iswhite[x]) whitedist[cur].insert(dist[cur][x]);
                else whitedist[cur].erase(whitedist[cur].find(dist[cur][x]));
                cur = parent[cur];
            }
        }
        else {
            ...
            int ans = inf;
            while (cur) {
                ans = min(ans, dist[cur][x] + *whitedist[cur].begin());
                cur = parent[cur];
            }
