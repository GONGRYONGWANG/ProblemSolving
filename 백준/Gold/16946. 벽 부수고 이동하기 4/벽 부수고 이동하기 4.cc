...
pii findp(pii node) {
    ...
    return parent[node.first][node.second] = findp(parent[node.first][node.second]);
}
void mer(pii a, pii b) {
    if (findp(a) == findp(b)) return;
    parent[findp(b).first][findp(b).second] = findp(a);
}
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            for (int dir = 0; dir < 4; dir++) {
                ...
                int ny = j + dy[dir];
                if (nx < 0 || nx >= N || ny < 0 || ny >= M || board[nx][ny]) continue;
                st.insert(findp({ nx,ny }));
            }
            for (auto iter = st.begin(); iter != st.end(); iter++) {
                ret += sz[(*iter).first][(*iter).second];
            }
            cout << ret % 10;
        }
        cout << endl;
