...
int board[4][4];
int DP[16][(1 << 16)];
int dp(int n, int hist) {
    if (n == N * M) return 0;
    if (DP[n][hist] != -1) return DP[n][hist];
    if (hist & (1 << n)) return dp(n + 1, hist);
    int x = n / M;
    int y = n % M;
    ...
    for (int dir = 2; dir < 4; dir++) {
        ...
        int nx = x; int ny = y;
        int nxhist = hist;
        while (nx >= 0 && nx < N && ny >= 0 && ny < M && !(hist & (1 << (nx * M + ny)))) {
            psum = psum * 10 + board[nx][ny];
            nxhist |= (1 << (nx * M + ny));
            ret = max(ret, psum + dp(n + 1, nxhist));
            nx += dx[dir]; ny += dy[dir];
        }
    }
