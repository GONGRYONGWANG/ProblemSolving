...
void solve() {
    ...
    while (!q.empty()) {
        ...
        if (x != N && !visited[x + 1][y][(d + b) % mod]) {
            visited[x + 1][y][(d + b) % mod] = true;
            q.push({ x + 1,y,(d + b) % mod });
        }
        if (y != N && x > y && !visited[x][y + 1][(d * c) % mod]) {
            visited[x][y + 1][(d * c) % mod] = true;
            q.push({ x,y + 1,(d * c) % mod });
        }
    }
    cout << ans;
