...
void solve() {
    ...
    dist[K] = 0;
    for (pii& e : E[K]) {
        q[e.second].push({ e.first, e.second });
    }

    while (1) {
        int n = 0;
        for (int i = 1; i <= 10; i++) {
            while (!q[i].empty() && dist[q[i].front().first] != inf) q[i].pop();
            if (q[i].empty()) continue;
            if (q[i].front().second < q[n].front().second) n = i;
        }
        if (n == 0) break;
        int cur = q[n].front().first;
        int d = q[n].front().second;
        q[n].pop();
        dist[cur] = d;
        for (pii& e : E[cur]) {
            if (dist[e.first] != inf) continue;
            q[e.second].push({ e.first, d + e.second });
        }
    }
