...
void solve() {
    ...
    while (!visited[N + 1]) {
        ...
        for (int i = 0; i <= N + 1; i++) {
            if (visited[i]) continue;
            ld ret = sqrt((arr[nx].first - arr[i].first) * (arr[nx].first - arr[i].first) + (arr[nx].second - arr[i].second) * (arr[nx].second - arr[i].second));
            if (nx != 0 && nx != N + 1) ret -= 1;
            if (i != 0 && i != N + 1) ret -= 1;
            ret = max(ld(0), ret);
            ret += dist[nx];
            dist[i] = min(dist[i], ret);
        }
