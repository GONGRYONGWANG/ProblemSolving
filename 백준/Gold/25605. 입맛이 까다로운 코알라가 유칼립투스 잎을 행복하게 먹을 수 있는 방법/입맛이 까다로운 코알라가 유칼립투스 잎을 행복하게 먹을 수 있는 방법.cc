...
pii arr[1000];
int DP[100][1000][101];
int dp(int n, int idx, int c) {
    if (n == M || idx == N) return 0;
    if (DP[n][idx][c] != -1) return DP[n][idx][c];
    int ret = max(dp(n, idx + 1, c), dp(n + 1, idx, max(0, c - B)));
    if (arr[idx].first + c <= A) ret = max(ret, arr[idx].second + dp(n + 1, idx + 1, max(0, c + arr[idx].first - B)));
    return DP[n][idx][c] = ret;
}
