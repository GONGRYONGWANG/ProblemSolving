...
void solve() {
    ...
    vector<pll> psum(N + 2); // sz, total
    psum[0] = { 0,0 };
    for (int i = 1; i <= N + 1; i++) {
        psum[i] = psum[i - 1];
        psum[i].first += arr[i].second;
        psum[i].second += arr[i].first * arr[i].second;
    }

    while (Q--) {
        ll x;
        cin >> x;
        auto it = lower_bound(arr.begin(), arr.end(), make_pair(x, 0ll));
        it--;
        int index = it - arr.begin();
        ll ret = 0;
        ret += x * psum[index].first - psum[index].second;
        ret += psum[N + 1].second - psum[index].second - (psum[N + 1].first - psum[index].first) * x;
        cout << ret << endl;
    }
