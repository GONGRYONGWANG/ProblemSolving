...
void lazyProp(int bck) {
    if (lazy[bck] == -1) return;
    for (int i = bck * 316; i < (bck + 1) * 316; i++) arr[i] = lazy[bck];
    lazy[bck] = -1;
}

void solve(int tc) {

    for (int i = 0; i < 317; i++) lazy[i] = -1;
    int N, Q;
    cin >> N >> Q;
    ...
    for (int i = 0; i < Q; i++) {
        int a, b, x;
        a = query[i][0]; b = query[i][1]; x = query[i][2];
        int lbck = a / 316; int rbck = b / 316;
        lazyProp(lbck); lazyProp(rbck);
        if (lbck == rbck) {
            for (int j = a; j <= b; j++) arr[j] = x;
            continue;
        }
        for (int j = a; j < (lbck + 1) * 316; j++) arr[j] = x;
        for (int bck = lbck + 1; bck < rbck; bck++) lazy[bck] = x;
        for (int j = rbck * 316; j <= b; j++) arr[j] = x;
    }
    for (int j = 0; j < 317; j++) lazyProp(j);
