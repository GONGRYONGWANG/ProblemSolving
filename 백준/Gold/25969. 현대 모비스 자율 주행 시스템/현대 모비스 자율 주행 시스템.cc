...
void solve() {
    ...
    while (!q.empty()) {
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            bool nb = b | (board[nx][ny] == 2);
            if (dist[nx][ny][k][nb]) continue;
            dist[nx][ny][k][nb] = dist[x][y][k][b] + 1;
            q.push({ nx,ny,k,nb });
        }
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                ...
                int nx = x + i - 2; int ny = y + j - 2;
                if (nx < 0 || nx >= N || ny < 0 || ny >= M) continue;
                if (board[nx][ny] == 1) continue;
                bool nb = b | (board[nx][ny] == 2);
                if (dist[nx][ny][k + 1][nb]) continue;
                dist[nx][ny][k + 1][nb] = dist[x][y][k][b] + 1;
                q.push({ nx,ny,k + 1,nb });
            }
        }
    }
    ...
    int ans = inf;
    for (int k = 0; k <= K; k++) {
        if (dist[N - 1][M - 1][k][1]) ans = min(ans, dist[N - 1][M - 1][k][1]);
    }
