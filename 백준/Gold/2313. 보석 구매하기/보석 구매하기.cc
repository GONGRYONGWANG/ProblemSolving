...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 1; j <= k; j++) {
            cin >> psum[j];
            psum[j] += psum[j - 1];
            if (psum[j] - psum[mn] > psum[ret.second] - psum[ret.first]) {
                ret = { mn,j };
            }
            else if (psum[j] - psum[mn] == psum[ret.second] - psum[ret.first] && j - mn < ret.second - ret.first) {
                ret = { mn,j };
            }

            if (psum[j] <= psum[mn]) mn = j;
        }
        ans.push_back(ret);
        total += psum[ret.second] - psum[ret.first];
    }
    cout << total << endl;
    for (pii x : ans) {
