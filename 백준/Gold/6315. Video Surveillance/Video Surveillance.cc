...
void solve() {
    ...
    while (N != 0) {
        ...
        arr[N] = arr[0];
        for (int i = 0; i < N; i++) {
            pll prv = arr[i]; pll cur = arr[i + 1];
            if (prv.first < cur.first) y.second = min(y.second, cur.second);
            else if (prv.first > cur.first) y.first = max(y.first, cur.second);
            else if (prv.second > cur.second) x.second = min(x.second, cur.first);
            else if (prv.second < cur.second) x.first = max(x.first, cur.first);
        }

        bool b = (x.first <= x.second && y.first <= y.second);
