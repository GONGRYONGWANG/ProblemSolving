...
int get(int l, int r) {
    ...
    if (l % 8) return get(l, (l / 8 + 1) * 8 - 1) + get((l / 8 + 1) * 8, r);

    if (r % 8 != 7) return get(l, r - r % 8 - 1) + get(r - r % 8, r);

    __m256i A = _mm256_setzero_si256();
    for (int i = l; i < r; i += 8) {
        __m256i b = _mm256_load_si256((__m256i*)(arr + i));
        A = _mm256_add_epi32(A, b);
    }

    _mm256_store_si256((__m256i*)tmp, A);
    for (int i = 0; i < 8; i++) ret += tmp[i];
    return ret;
...
void solve() {
    ...
    while (M--) {
        ...
        else if (t == 2) cout << (b - a + 1) - get(a - 1, b - 1) << endl;
