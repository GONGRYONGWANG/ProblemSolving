...
vector<vector<int>> DPy;
vector<int> DPx;
int dpy(int x, int y) {
    if (y >= M) return 0;
    if (DPy[x][y]) return DPy[x][y];
    return DPy[x][y] = max(dpy(x, y + 1), board[x][y] + dpy(x, y + 2));
}

int dpx(int x) {
    if (x >= N) return 0;
    if (DPx[x]) return DPx[x];
    return DPx[x] = max(dpx(x + 1), dpy(x, 0) + dpx(x + 2));
}
