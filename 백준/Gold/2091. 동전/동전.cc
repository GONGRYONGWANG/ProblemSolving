...
void solve() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int k = 1; k <= n; k *= 2) {
            n -= k;
            for (int j = X - w * k; j >= 0; j--) {
                if (!DP2[j].empty() && DP[j] + k > DP[j + w * k]) {
                    DP[j + w * k] = max(DP[j + w * k], k + DP[j]);
                    DP2[j + w * k] = DP2[j];
                    DP2[j + w * k][i] += k;
                }
            }
        ...
        if (n != 0) {
            for (int j = X - w * n; j >= 0; j--) {
                if (!DP2[j].empty() && DP[j] + n > DP[j + w * n]) {
                    DP[j + w * n] = max(DP[j + w * n], n + DP[j]);
                    DP2[j + w * n] = DP2[j];
                    DP2[j + w * n][i] += n;
                }
            }
