...
void solve() {
    ...
    for (int i = 0; i <= M; i++) {
        ...
        queue<vector<int>> q;
        q.push({ arr[i].first,arr[i].second,0 });
        visited[arr[i].first][arr[i].second] = true;
        while (!q.empty()) {
            int x = q.front()[0];
            int y = q.front()[1];
            int d = q.front()[2];
            q.pop();
            if (board[x][y] >= 0) adj[i][board[x][y]] = d;
