...
void solve() {
    ...
    for (int j = 0; j < 3; j++) {
        ...
        for (int i = 1; i <= N; i++) {
            arr[j][i].first = max(arr[j][i - 1].first, arr[j][i].first);
        }
    }
    ...
    for (int i = 0; i <= N; i++) {
        int r = N;
        for (int l = 0; l <= N; l++) {
            if (arr[0][i].second + arr[1][l].second > T) break;
            while (arr[0][i].second + arr[1][l].second + arr[2][r].second > T) {
                r -= 1;
            }
            ans = max(ans, arr[0][i].first + arr[1][l].first + arr[2][r].first);
        }
    }
