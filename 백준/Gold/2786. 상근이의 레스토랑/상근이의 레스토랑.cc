...
pll arr[500000];
bool visited[500000];
pq<pll, vector<pll>, greater<pll>> A;
pq<pll, vector<pll>, greater<pll>> B;
pq<ll, vector<ll>, greater<ll>> C;
void solve() {
    int N;
    cin >> N;
    for (int i = 0; i < N; i++) {
    ...
    ll ret = 0;
    for (int i = 0; i < N; i++) {
        while (!A.empty() && visited[A.top().second]) A.pop();
        ll ans = ret + A.top().first;

        ret += B.top().first;

        int idx = B.top().second;
        B.pop();
        visited[idx] = true;
        C.push(arr[idx].first - arr[idx].second);

        ans = min(ans, ret + C.top());
