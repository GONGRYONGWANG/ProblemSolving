...
void solve() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        cin >> u >> v >> T;
        edge[i] = { u,v };
        E[u][0].push_back({ v,0,T });
        E[v][0].push_back({ u,0,T });
    }
    ...
    for (int i = 1; i <= K; i++) {
        for (int j = 0; j < M; j++) {
            ...
            cin >> T;
            int u, v;
            u = edge[j].first; v = edge[j].second;
            E[u][i].push_back({ v,i,T });
            E[v][i].push_back({ u,i,T });
        }
        for (int j = 1; j <= N; j++) {
            E[j][i - 1].push_back({ j,i,0 });
        }
    }
    ...
    while (!q.empty()) {
        ...
        ll d = q.top()[2];
        q.pop();
        if (visited[cur][k] != -1) continue;
        visited[cur][k] = d;
        for (vector<ll> nx : E[cur][k]) {
            int nxt = nx[0];
            int nk = nx[1];
            ll nd = d + nx[2];
            if (visited[nxt][nk] != -1) continue;
            q.push({ nxt,nk,nd });
        }
    }
