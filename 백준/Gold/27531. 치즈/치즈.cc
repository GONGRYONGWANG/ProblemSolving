...
void solve() {
    ...
    for (int i = 1; i <= N; i++) {
        ...
        visited[i] = true;
        int x = E[i][0].first;
        arr.push_back(E[i][1].second);
        arr.push_back(E[i][0].second);
        while (x!=E[i][1].first) {
            visited[x] = true;
            if (!visited[E[x][0].first]) {
                arr.push_back(E[x][0].second);
                x = E[x][0].first;
            }
            else {
                arr.push_back(E[x][1].second);
                x = E[x][1].first;
            }
        }
        visited[x] = true;
        ...
        DP[sz - 1][0] = 0; DP[sz - 1][1] = arr[sz - 1];
        DP[sz - 2][0] = arr[sz - 2]; DP[sz - 2][1] = arr[sz - 1];
        for (int i = sz - 3; i > 0; i--) {
            DP[i][0] = min(arr[i] + DP[i + 1][0], arr[i + 1] + DP[i + 2][0]);
            DP[i][1] = min(arr[i] + DP[i + 1][1], arr[i + 1] + DP[i + 2][1]);
        }
        DP[0][0] = arr[0] + DP[1][0]; DP[0][1] = min(arr[0] + DP[1][1], arr[1] + DP[2][1]);
