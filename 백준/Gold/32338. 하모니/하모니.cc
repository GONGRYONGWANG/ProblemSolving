...
void solve() {
    ...
    for (int i = 0; i < T; i++) {
        ...
        for (int j = N - w; j >= 0; j--) {
            for (int i = 0; i <= 200; i++) {
                if (i + m < 0 || i + m>200) continue;
                DP[i + m][j + w] = max(DP[i + m][j + w], DP[i][j] + v);
            }
        }
    }
    ...
    for (int i = 0; i <= 200; i++) {
        for (int j = 0; j <= N; j++) {
            ...
            else if (DP[i][j] == DP[ans.first][ans.second]) {
                if (abs(i - 100) < abs(ans.first - 100)) {
                    ans = { i,j };
                }
                else if (abs(i - 100) == abs(ans.first - 100) && i > ans.first) {
                    ans = { i,j };
                }
            }
        }
    ...
    cout << ans.first - 100 << " " << DP[ans.first][ans.second];
