...
string parseExpr(const string& s, int& i);

string parseFactor(const string& s, int& i) {
    if (s[i] == '(') {
        i++;  
        string expr = parseExpr(s, i);
        i++; 
        return "(" + expr + ")";
    }
    else {
...
string parseTerm(const string& s, int& i) {
    string term = parseFactor(s, i);
    while (i < s.size() && s[i] == '*') {
        i++;  
        string factor = parseFactor(s, i);
        term = "(" + term + "*" + factor + ")";
    }
    return term;
}
...
string parseExpr(const string& s, int& i) {
    string expr = parseTerm(s, i);
    while (i < s.size() && s[i] == '+') {
        i++;  
        string term = parseTerm(s, i);
        expr = "(" + expr + "+" + term + ")";
    }
    return expr;
}
