...
void solve() {
    ...
    for (int t = 1; t <= T; t++) {
        ...
        int prv = -1;
        for (int i = 30; i >= 0; i--) {
            if (!onetile[i]) continue;
            if (prv == -1) {
                onetile[i] = 1;
            }
            else {
                onetile[i] = ((onetile[prv] - 1) / 2 + 1) * (1ll << (prv - i)) * 2 + 1;
            }
            prv = i;
        }
        ...
        int ans = 0;
        vector<int> stock(31, 0);
        for (int i = 30; i >= 0; i--) {
            while (arr[i] > stock[i]) {
                for (int j = 0; j <= i; j++) {
                    stock[j] = min(stock[j] + onetile[j], 1000ll);
                }
                ans += 1;
            }
            if (i != 0) {
                stock[i - 1] = min(stock[i - 1] + 4 * (stock[i] - arr[i]), 1000ll);
                onetile[i - 1] = min(onetile[i - 1] + onetile[i] * 4, 1000ll);
            }
        }
