...
void solve(int tc) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        cin >> a >> b >> c;
        if (i == 0) r = (a + b + c) % 2;
        if ((a + b + c) % 2 != r) x = false;
        w = { max(w.first, a + b - c),min(w.second, a + b + c) };
        h = { max(h.first, a - b - c), min(h.second, a - b + c) };
    }

    if (w.first > w.second || h.first > h.second) x = false;
    if (!x) cout << "NO";
    else cout << (w.first + h.first) / 2 << " " << (w.first - h.first) / 2;
