...
ll mod = 1e9 + 7;
ll DP[3001][3001][2];
ll dp(int n, int prv, bool b) {
    if (prv == K && !b) return dp(n, prv, true);
    if (n == N) {
        if (b) return 1;
        else return 0;
    }
    if (DP[n][prv][b] != -1) return DP[n][prv][b];

    if (arr[n]) {
        if (prv == K) return DP[n][prv][b] = 0;
        else return DP[n][prv][b] = dp(n + 1, prv + 1, b);
    }

    if (prv == K) return DP[n][prv][b] = dp(n + 1, 0, b);
    else return DP[n][prv][b] = (dp(n + 1, 0, b) + dp(n + 1, prv + 1, b)) % mod;
