...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < M; j++) {
            ...
            sum[i] += x;
            psum += x;
            lmx[i] = max(psum, lmx[i]);
            rmx[i] = min(psum, rmx[i]);
            ans = max(ans, psum - rmx[i]);
        }
        rmx[i] = sum[i] - rmx[i];
        if (sum[i] > 0) totalsum += sum[i];
    }

    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if (i == j) continue;
            int ret = rmx[i] + lmx[j] + totalsum;
            if (sum[i] > 0) ret -= sum[i];
            if (sum[j] > 0) ret -= sum[j];
            ans = max(ans, ret);
        }
    }
    cout << ans;
