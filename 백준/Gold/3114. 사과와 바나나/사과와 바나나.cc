...
int dp(int x, int y) {
    ...
    int a, b;
    b = psumB[x - 1][y];
    a = psumA[N][y] - psumA[x][y];
    if (x == N && y == M) return DP[x][y] = b + a;
    int ret = b + a + max(dp(x, y + 1), dp(x + 1, y + 1));

    ret = max(ret, dp(x + 1, y) - AB[x][y] * board[x][y]);
...
void solve(int tc) {
    ...
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= M; j++) {
            ...
            AB[i][j] = (x == 'B');
            cin >> board[i][j];
            psumB[i][j] = psumB[i - 1][j] + AB[i][j] * board[i][j];
            psumA[i][j] = psumA[i - 1][j] + (!AB[i][j]) * board[i][j];
        }
    }
