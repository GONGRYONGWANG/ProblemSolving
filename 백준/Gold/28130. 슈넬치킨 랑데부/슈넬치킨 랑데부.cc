...
void solve() {
    ...
    while (!q.empty()) {
        ...
        if (A[x][y] != -1) {
            int ret = B[x][y];
            while(ret<A[x][y]) ret+= 2 * (N + M - 2);
            ans = min(ans, ret);
        }

        pii nxt;
        if (x == 0 && y != M - 1) nxt = { x, y + 1 };
        else if (y == M - 1 && x != N - 1) nxt = { x + 1,y };
        else if (x == N - 1 && y != 0) nxt = { x, y - 1 };
        else nxt = { x - 1, y };
        if (nxt == b) break;
        B[nxt.first][nxt.second] = B[x][y] + 1;
        q.push(nxt);
    }
