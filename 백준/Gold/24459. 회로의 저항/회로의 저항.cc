...
pii dfs(int x, int p) {
    ...
    for (pii& e : E[x]) {
        ...
        pii ret = dfs(e.first, x);
        ans.first = min(ans.first, mn + ret.first + e.second);
        ans.second = max(ans.second, mx + ret.second + e.second);
        mn = min(mn, ret.first + e.second);
        mx = max(mx, ret.second + e.second);
    }
    return { mn,mx };
}
