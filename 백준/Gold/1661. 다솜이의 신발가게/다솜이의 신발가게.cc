...
void solve() {
    ...
    for (int a = 0; a <= arr[1].size(); a++) {
        for (int b = 0; b <= arr[2].size(); b++) {
            for (int c = 0; c <= arr[3].size(); c++) {
                ld ret = P * pow(0.99, a) * pow(0.98, b) * pow(0.97, c);
                if (a != 0) ret += arr[1][a - 1];
                if (b != 0) ret += arr[2][b - 1];
                if (c != 0) ret += arr[3][c - 1];
                ans = min(ans, ret);
            }
        }
    }
