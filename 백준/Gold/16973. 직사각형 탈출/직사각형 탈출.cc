...
void solve() {
    ...
    for (int i = 1; i <= N - H + 1; i++) {
        for (int j = 1; j <= M - W + 1; j++) {
            for (int k = i; k < i + H; k++) {
                if (psum[k][j + W - 1] - psum[k][j - 1]) visited[i][j] = inf;
            }
        }
    }
    ...
    while (!q.empty()) {
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            if (nx == 0 || nx > N - H+1 || ny == 0 || ny > M - W+1 || visited[nx][ny]) continue;
            visited[nx][ny] = visited[x][y] + 1;
            q.push({ nx,ny });
        }
    }
    if (visited[tx][ty] && visited[tx][ty] != inf) cout << visited[tx][ty] - 1;
    else cout << -1;
