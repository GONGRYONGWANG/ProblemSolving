...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            if (board[i][j - 1] && !board[i][j]) colwall[j] += 1;
            else { ans += colwall[j] / 2; colwall[j] = 0; }

            if (!board[i][j - 1] && board[i][j]) colwall[j + M] += 1;
            else { ans += colwall[j + M] / 2; colwall[j + M] = 0; }

            if (board[i - 1][j] && !board[i][j]) rowwall[i] += 1;
            else { ans += rowwall[i] / 2; rowwall[i] = 0; }

            if (!board[i - 1][j] && board[i][j]) rowwall[i + N] += 1;
            else { ans += rowwall[i + N] / 2; rowwall[i + N] = 0; }
        }
    }
