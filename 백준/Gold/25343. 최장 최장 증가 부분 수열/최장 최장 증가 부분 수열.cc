...
vector<int> dp(int x, int y) {
    if (x == 0 || y == 0) return {};
    if (!DP[x][y].empty()) return DP[x][y];
    dp(x - 1, y); dp(x, y - 1);
    vector<int> ret(max(DP[x - 1][y].size(), DP[x][y - 1].size()), inf);

    for (int i = 0; i < DP[x - 1][y].size(); i++) {
        ret[i] = min(ret[i], DP[x - 1][y][i]);
    }
    for (int i = 0; i < DP[x][y-1].size(); i++) {
        ret[i] = min(ret[i], DP[x][y - 1][i]);
    }
    auto it = lower_bound(ret.begin(), ret.end(), board[x][y]);
    if (it == ret.end()) ret.push_back(board[x][y]);
    else *it = board[x][y];
    return DP[x][y] = ret;
