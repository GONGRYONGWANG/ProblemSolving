...
void solve() {
    ...
    while (!q.empty()) {
        ...
        q.pop();
        if (fromA[cur]!=-1) continue;
        fromA[cur] = dist;
        for (pii nx : E[cur]) {
            if (fromA[nx.first]!=-1) continue;
            q.push({ nx.first,nx.second + dist });
        }
    }
    ...
    while (!q.empty()) {
        ...
        int dist = q.top().second;
        q.pop();
        if (fromB[cur]!=-1) continue;
        fromB[cur] = dist;
        for (pii nx : E[cur]) {
            if (fromB[nx.first]!=-1) continue;
            q.push({ nx.first,nx.second + dist });
        }
    }
