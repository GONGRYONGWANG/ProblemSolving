...
void solve() {
    ...
    while (K--) {
        ...
        if (t == 1) {
            b -= 1;
            int bucketnum = b / 1000;
            for (int i = bucketnum * 1000; i < (bucketnum + 1) * 1000; i++) {
                arr[i] += bucketextra[bucketnum];
            }
            bucketextra[bucketnum] = 0;
            bucketsum[bucketnum] += c - arr[b];
            arr[b] = c;
        }
        else {
            b -= 1; c -= 1;
            int l = b / 1000; int r = c / 1000;
            for (int i = l * 1000; i < (l + 1) * 1000; i++) {
                arr[i] += bucketextra[l];
            }
            bucketextra[l] = 0;
            for (int i = r * 1000; i < (r + 1) * 1000; i++) {
                arr[i] += bucketextra[r];
            }
            bucketextra[r] = 0;
            ...
            if (l == r) {
                ...
                ll ret = 0;
                for (int i = b; i < (l+1)*1000; i++) {
                    ret += arr[i];
                }
                for (int i = r * 1000; i <= c; i++) {
                    ret += arr[i];
                }
                for (int i = l + 1; i < r; i++) {
                    ret += bucketsum[i];
