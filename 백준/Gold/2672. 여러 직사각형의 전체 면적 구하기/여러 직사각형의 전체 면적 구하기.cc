...
void solve() {
    ...
    vector<pair<ld,ld>> vline;
    vline.push_back({ 0,0 });
    map<ld, vector<int>> m;
    for (int i = 0; i < N; i++) {
        ld x, y, w, h;
        cin >> x >> y >> w >> h;
        vline.push_back({ y, y + h });
        m[x].push_back(vline.size() - 1);
        m[x + w].push_back(-(int(vline.size()) - 1));
    }


    ld ans = 0;

    multiset<pair<ld,ld>> lines;
    ...
    for (auto it = m.begin(); it != m.end(); it++) {
        ...
        ld w = nit->first - it->first;

        for (int x : it->second) {
            if (x < 0) lines.erase(lines.find(vline[-x]));
            else lines.insert(vline[x]);
        }
