...
void solve(int tc) {
    ...
    while (!q.empty()) {
        int x = q.front()[0]; int y = q.front()[1]; int key = q.front()[2];
        q.pop();
        for (int dir = 0; dir < 4; dir++) {
            int nx = x + dx[dir]; int ny = y + dy[dir]; int nkey = key;
            if (nx < 0 || nx >= N || ny < 0 || ny >= M) continue;
            if (board[nx][ny] == -2) continue;
            if (board[nx][ny] >= 0) nkey |= (1 << board[nx][ny]);
    ...
    cout << visited[t.first][t.second][(1 << cnt) - 1] - 1;
