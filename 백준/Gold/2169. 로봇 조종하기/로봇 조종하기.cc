...
ll dp(int x, int y, int d) {
    ...
    if (d == 0) return DP[x][y][d] = board[x][y] + max(max(dp(x, y - 1, 1), dp(x, y + 1, 2)), dp(x + 1, y, 0));
    if (d == 1) return DP[x][y][d] = board[x][y] + max(dp(x, y - 1, 1), dp(x + 1, y, 0));
    else return DP[x][y][d] = board[x][y] + max(dp(x, y + 1, 2), dp(x + 1, y, 0));
}
