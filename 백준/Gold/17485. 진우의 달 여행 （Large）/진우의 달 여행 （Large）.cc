...
int dp(int x, int y, int p) {
    ...
    DP[x][y][p] = inf;
    if (p != 0) DP[x][y][p] = min(DP[x][y][p], dp(x + 1, y - 1, 0) + board[x][y]);
    if (p != 1) DP[x][y][p] = min(DP[x][y][p], dp(x + 1, y, 1) + board[x][y]);
    if (p != 2) DP[x][y][p] = min(DP[x][y][p], dp(x + 1, y + 1, 2) + board[x][y]);
    return DP[x][y][p];
}
