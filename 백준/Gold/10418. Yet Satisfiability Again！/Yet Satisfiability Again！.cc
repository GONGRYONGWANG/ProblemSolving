...
void solve() {
    ...
    while (M--) {
        ...
        while (idx < s.length()) {
            if (s[idx] == '~') {
                idx += 2;
                int tmp = (int)s[idx] - 48;
                if (idx != s.length() - 1 && isdigit(s[idx + 1])) {
                    idx += 1;
                    tmp = tmp * 10 + (int)s[idx] - 48;
                }
                idx += 4;
                v.push_back(-tmp);
            }
        ...
        for (int i : candidate) {
            ...
            if (ret) nxtcand.push_back(i);
        }
        candidate = nxtcand;
    }
    cout << ((candidate.size() != 0) ? "satisfiable" : "unsatisfiable") << endl;
