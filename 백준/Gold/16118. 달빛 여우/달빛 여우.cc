...
void solve() {
    ...
    while (!q.empty()) {
        ...
        fox[cur] = dist;
        for (pll& e : E[cur]) {
            if (fox[e.first]) continue;
            q.push({ e.first,e.second * 2 + dist });
        }
    }
    ...
    while (!q.empty()) {
        ...
        bool t = q.top()[2];
        q.pop();
        if (wolf[cur][t]) continue;
        wolf[cur][t] = dist;
        for (pll& e : E[cur]) {
            if (wolf[e.first][!t]) continue;
            ll nd = dist;
            if (!t) nd += e.second;
            else nd += 4 * e.second;
            q.push({ e.first, nd , !t });
    ...
    int ans = 0;
    for (int i = 2; i <= N; i++) {
        if (wolf[i][1] == 0 || wolf[i][0] == 0) {
            wolf[i][0] = max(wolf[i][0], wolf[i][1]);
        }
        else wolf[i][0] = min(wolf[i][0], wolf[i][1]);
        if (fox[i] < wolf[i][0]) ans += 1;
    }
    cout << ans;
