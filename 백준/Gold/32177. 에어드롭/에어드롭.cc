...
void solve() {
    ...
    while (!q.empty()){
        int cur = q.front();
        q.pop();
        if (arr[cur][3]) ans.push_back(cur);
        x = arr[cur][0]; y = arr[cur][1]; v = arr[cur][2];
        for (int i = 1; i <= N; i++) {
            if (visited[i]) continue;
            int nx = arr[i][0], ny = arr[i][1], nv = arr[i][2];
            if ((x - nx) * (x - nx) + (y - ny) * (y - ny) > K * K) continue;
            if (abs(v - nv) > T) continue;
            visited[i] = true;
            q.push(i);
        }
    }
