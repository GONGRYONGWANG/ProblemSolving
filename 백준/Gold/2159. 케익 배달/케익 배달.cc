...
void solve() {
    ...
    for (int dir = 0; dir < 5; dir++) {
        int nx = x + dx[dir];
        int ny = y + dy[dir];
        dist[dir] = abs(nx - sx) + abs(ny - sy);
    }

    for (int i = 0; i < N - 1; i++) {
        int prvx = x; int prvy = y;
        cin >> x >> y;
        vector<ll> nxtdist(5, INF);
        for (int dir = 0; dir < 5; dir++) {
            for(int dir2 = 0; dir2 < 5; dir2++){
                int px = prvx + dx[dir]; int py = prvy + dy[dir];
                int nx = x + dx[dir2]; int ny = y + dy[dir2];
                nxtdist[dir2] = min(nxtdist[dir2], dist[dir] + abs(nx - px) + abs(ny - py));
            }
        }
        dist = nxtdist;
    }
