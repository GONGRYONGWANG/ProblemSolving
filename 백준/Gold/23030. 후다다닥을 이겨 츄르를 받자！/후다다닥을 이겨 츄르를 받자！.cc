...
void solve() {
    ...
    for (int i = 0; i < M; i++) {
        int p1, p2, q1, q2;
        cin >> p1 >> p2 >> q1 >> q2;
        E[p1][p2].push_back({ q1,q2 });
        E[q1][q2].push_back({ p1,p2 });
    }
    int K;
    cin >> K;
    while (K--) {
        ...
        int u1, u2, v1, v2;
        cin >> u1 >> u2 >> v1 >> v2;
        unordered_set<pii,PiiHasher> visited;
        pq<vector<int>, vector<vector<int>>, cmp> q;
        q.push({ u1,u2,0 });
        while (!q.empty()) {
            pii cur = { q.top()[0],q.top()[1] };
            int d = q.top()[2];
            q.pop();
            if (visited.count(cur)) continue;
            visited.insert(cur);
            if (cur == make_pair(v1,v2)) {
                cout << d << endl;
                break;
            }
            if (cur.second != 1 && !visited.count({ cur.first,cur.second - 1 })) {
                q.push({ cur.first,cur.second - 1, d + 1 });
            }
            if (cur.second != L[cur.first] && !visited.count({ cur.first, cur.second + 1 })) {
                q.push({ cur.first,cur.second + 1,d + 1 });
            }
            for (pii& nx : E[cur.first][cur.second]) {
                if (!visited.count(nx)) {
                    q.push({ nx.first,nx.second,d + t });
                }
            }
        }
    }
