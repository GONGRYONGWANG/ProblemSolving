...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (i != 0 && arr[i].first > arr[i - 1].first) {
            cur.first = min(cur.first, tmp.first);
            cur.second = max(cur.second, tmp.second);
        }
        tmp.first = min(tmp.first, y);
        tmp.second = max(tmp.second, y);
        if (cur.first >= y || cur.second <= y) ret[i] = false;
    }
