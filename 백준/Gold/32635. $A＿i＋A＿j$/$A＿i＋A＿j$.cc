...
vector<pll> E[1000001];
vector<pll> rE[1000001];
...
struct cmp2 {
    bool operator()(vector<ll>& a, vector<ll>& b) {
        if (a[1] == b[1] && a[0] == b[0]) return a[2] > b[2];
...
void solve() {
    ...
    while (!q.empty()) {
        ...
        ll d = q.top().second;
        q.pop();
        if (distT[cur] != -1) continue;
        distT[cur] = d;
        for (pll& e : rE[cur]) {
            int nx = e.first;
            if (distT[nx] != -1) continue;
            q.push({ nx,e.second + d });
        }
    }
    ...
    pq<vector<ll>, vector<vector<ll>>, cmp2> q2;
    q2.push({ S,0,inf });
    while (!q2.empty()) {
        int cur = q2.top()[0];
        ll d = q2.top()[1];
        ...
        visited[cur] = true;
        ans = min(ans, mn + arr[cur]);
        mn = min(mn, arr[cur]);
        for (pll& e : E[cur]) {
            int nx = e.first;
            if (visited[nx]) continue;
            if (distT[nx] == -1) continue;
            if (d + e.second + distT[nx] != distT[S]) continue;
            q2.push({ e.first,d + e.second, mn });
        }
    }
