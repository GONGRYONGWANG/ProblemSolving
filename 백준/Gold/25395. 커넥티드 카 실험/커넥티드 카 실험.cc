...
void solve() {
    ...
    int lidx = S;
    int ridx = S;
    while ((lidx != 0 && arr[lidx] >= l) || (ridx != N + 1 && arr[ridx] <= r)) {
        while (lidx != 0 && arr[lidx] >= l) {
            l = min(l, arr[lidx] - h[lidx]);
            r = max(r, arr[lidx] + h[lidx]);
            lidx -= 1;
        }
        while (ridx != N + 1 && arr[ridx] <= r) {
            l = min(l, arr[ridx] - h[ridx]);
            r = max(r, arr[ridx] + h[ridx]);
            ridx += 1;
        }
    }
