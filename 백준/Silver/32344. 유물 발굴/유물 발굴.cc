...
void solve() {
    ...
    vector<vector<ll>> arr(N + 1, { R,1,C,1 });
    for (int i = 0; i < N; i++) {
        ll a, v, h;
        cin >> a >> v >> h;

        arr[a][0] = min(arr[a][0], v);
        arr[a][1] = max(arr[a][1], v);
        arr[a][2] = min(arr[a][2], h);
        arr[a][3] = max(arr[a][3], h);
        if ((arr[a][1] - arr[a][0] + 1) * (arr[a][3] - arr[a][2] + 1) > mx) {
            mx = (arr[a][1] - arr[a][0] + 1) * (arr[a][3] - arr[a][2] + 1);
            ans = a;
        }
        else if ((arr[a][1] - arr[a][0] + 1) * (arr[a][3] - arr[a][2] + 1) == mx) {
            ans = min(ans, a);
        }
    }
