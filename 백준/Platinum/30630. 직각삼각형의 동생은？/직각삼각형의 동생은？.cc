...
void solve(int tc) {
    ...
    for(int i=0; i<M; i++){
        ll a = query[i][0]; ll b = query[i][1]; int idx = query[i][2];
        while(!arr.empty() && !cmp(arr.back(), {a,b})){
            os.insert(arr.back().first);
            arr.pop_back();
        }
        ret[idx] = os.order_of_key(a+1);
    }
