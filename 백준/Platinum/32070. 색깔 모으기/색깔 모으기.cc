...
void solve() {
    ...
    int ans = 0;
    for (int i = 1; i <= N; i++) {
        if (topcnt[i] + botcnt[i] == 2) continue;
        int tt = 0;
        int bb = 0;
        int n = 0;
        queue<int> q;
        ...
        while (!q.empty()) {
            ...
            for (pii e : E[cur]) {
                ...
                n += 1;
                visit[e.second] = true;
                topcnt[arr[e.second].first] += 1;
                botcnt[arr[e.second].second] += 1;
                if (topcnt[arr[e.second].first] == 2) tt += 1;
                if (botcnt[arr[e.second].second] == 2) bb += 1;
                q.push(e.first);
            }
        }
        if ((tt == 0 && bb == 0) || (tt == 1 && bb == 1)) {
            if (n == 1) ans += 0;
            else ans += n + 1;
        }
        else {
