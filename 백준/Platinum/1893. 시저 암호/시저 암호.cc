...
void solve() {
    ...
    for (int i = 1; i < W.size(); i++) {
        pattern.push_back(char((m[W[i]] + N - m[W[i - 1]]) % N + 48));
    }
    ...
    string txt;
    for (int i = 1; i < S.size(); i++) {
        txt.push_back(char((m[S[i]] + N - m[S[i - 1]]) % N + 48));
    }

    vector<int> ret = kmp(pattern, txt, pi);

    for (int x : ret) {
        if (m[W[0]] <= m[S[x]]) {
            x = m[S[x]] - m[W[0]];
        }
        else x = m[S[x]] + N - m[W[0]];
        cnt[x] += 1;
    }
