...
struct HopcroftKarp {
    ...
    // 생성자
    HopcroftKarp(int V) : V(V), adj(V + 1), NIL(0), INF(V + 1) {
        pairU.resize(V + 1, NIL);
...
void solve() {
    ...
    HopcroftKarp graph(2 * N);
    for (int i = 1; i <= N; i++) {
        int v = i + N;
        if (arr[i] + arr[v] <= W) graph.addEdge(i, v);
        v = (i % N) ? (i + 1) : (i - N + 1);
        if (arr[i] + arr[v] <= W) graph.addEdge(i, v);
    }
    ...
    cout << 2 * N - graph.maxMatching() / 2 << endl;
