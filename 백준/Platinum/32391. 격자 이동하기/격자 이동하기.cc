...
ll cal(ll a, ll b) {
    return F[a + b] * Finv[a] % mod * Finv[b] % mod;
}
...
void solve() {
    ...
    m[{0, 0}] = { 0,1 };
    for (int i = 0; i < N; i++) {
        ll x = arr[i].first; ll y = arr[i].second;

        ll mx = 0;
        ll cnt = 0;

        for (auto it = m.begin(); it != m.end(); it++) {
            ll px = (it->first).first;
            ll py = (it->first).second;
            if (px > x) break;
            if (py > y) continue;
            ...
            if ((it->second).first == mx) {
                cnt += (it->second).second * cal(x - px, y - py) % mod;
                cnt %= mod;
            }
        }
        mx += 1;
        x += 1;
        y += 1;
        if (!m.count({ x,y }) || m[{x, y}].first < mx) {
            m[{x, y}] = { mx, cnt };
        }
        else if (m[{x, y}].first == mx) {
            m[{x, y}].second += cnt;
            m[{x, y}].second %= mod;
        }
    }
