...
vector<vector<ll>> powmat(const vector<vector<ll>>& mat, ll n) {
    if (n == 1) return mat;
    if (n % 2) {
        return matmul(powmat(mat, n - 1), mat);
    }
    vector<vector<ll>> half = powmat(mat, n / 2);
...
void solve() {
    ...
    vector<vector<ll>> ret = matmul(powmat({ {1,1},{1,0} }, N - 1),  {{1},{0}} );
    cout << ret[0][0];
