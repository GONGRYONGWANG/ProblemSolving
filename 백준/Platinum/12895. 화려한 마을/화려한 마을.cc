...
void lazyProp(int node, int start, int end) {
    ...
    for (int i = 1; i <= T; i++) {
        if (i == lazy[node]) tree[node][i] = true;
        else tree[node][i] = false;
    }
    if (start != end) {
...
bool get(int node, int start, int end, int left, int right, int val) {
    lazyProp(node, start, end);
    if (left > end || right < start) return false;
    if (left <= start && right >= end) return tree[node][val];
    int mid = (start + end) / 2;
