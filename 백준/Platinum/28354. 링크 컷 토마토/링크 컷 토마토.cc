...
void solve(int tc) {
    ...
    for (int i = 0; i < Q; i++) {
        int t, u, v;
        cin >> t >> u >> v;
        query.push_back({ t,0,u,v });
    }

    sort(query.begin(), query.end());
    unordered_map<pii, int, PiiHasher> cnt;
    for (int i = 0; i < query.size(); i++) {
        int u = query[i][2]; int v = query[i][3];
        cnt[{u, v}] += 1;
        cnt[{u, v}] %= 2;
        query[i][1] = cnt[{u, v}];
    }
    ...
    for (int t = 0; t <= 400000; t++) {
        ...
        while (idx != query.size() && query[idx][0] == t) {
            int u = query[idx][2]; int v = query[idx][3];
            if (query[idx][1] == 0) {
