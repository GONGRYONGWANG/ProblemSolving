...
pll mer(pll t1, pll t2) {
    if (t1.first > t2.first) return t1;
    if (t1.first < t2.first) return t2;
    return { t1.first,(t1.second + t2.second) % mod };
}
...
void update(int node, int start, int end, int x, pll val) {
...
void solve(int tc) {
    ...
    comp[Y[0]] = 1;
    for (int i = 1; i < N; i++) {
        if (Y[i] > Y[i - 1]) comp[Y[i]] = comp[Y[i - 1]] + 1;
    }

    for (int i = 0; i < N; i++) {
        int y = comp[arr[i].second];
        pll g = { 0,1 };
        if (y != 1) g = get(1, 1, N, 1, y - 1);
        if (g.second == 0) g = { 0,1 };
        g.first += 1;
        update(1, 1, N, y, g);
    }

    cout << tree[1].first << endl << tree[1].second;
