...
void solve(int tc) {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            if (i == 0 || j == 0 || board[i][j] != board[i - 1][j] || board[i][j] != board[i][j - 1]) {
                DP[i][j] = 0; 
                continue;
            }
    ...
    for (int i = 0; i < N; i++) {
        for (int j = M - 1; j >= 0; j--) {
            if (i == 0 || j == M - 1 || board[i][j] != board[i - 1][j] || board[i][j] != board[i][j + 1]) {
                DP[i][j] = 0;
                continue;
            }
            DP[i][j] = min(DP[i - 1][j], DP[i][j + 1]) + 1;
    ...
    for (int i = N - 1; i >= 0; i--) {
        for (int j = 0; j < M; j++) {
            if (i == N - 1 || j == 0 || board[i][j] != board[i + 1][j] || board[i][j] != board[i][j - 1]) {
                DP[i][j] = 0;
                continue;
            }
            DP[i][j] = min(DP[i + 1][j], DP[i][j - 1]) + 1;
    ...
    for (int i = N - 1; i >= 0; i--) {
        for (int j = M - 1; j >= 0; j--) {
            if (i == N - 1 || j == M - 1 || board[i][j] != board[i + 1][j] || board[i][j] != board[i][j + 1]) {
                DP[i][j] = 0;
                continue;
            }
            DP[i][j] = min(DP[i + 1][j], DP[i][j + 1]) + 1;
