...
int dp(int x, int p) {
    ...
    if (arr[x] >= p) {
        DP[x][p] = min(arr[x] - p + dp(x + 1, arr[x]), (10 - arr[x] + p) + dp(x + 1, p));
    }
    else {
        DP[x][p] = min((10 - p + arr[x]) + dp(x + 1, arr[x]), (p - arr[x]) + dp(x + 1, p));
    }
...
void backtrack(int x, int p) {
    ...
    cout << x + 1 << " ";
    if (arr[x] >= p) {
        if (dp(x, p) == arr[x] - p + dp(x + 1, arr[x])) {
            cout << arr[x] - p << endl;
            backtrack(x + 1, arr[x]);
        }
        else {
            cout << -(10 - arr[x] + p) << endl;
            backtrack(x + 1, p);
        }
    }
    else {
        if (dp(x, p) == (10 - p + arr[x]) + dp(x + 1, arr[x])) {
            cout << (10 - p + arr[x]) << endl;
            backtrack(x + 1, arr[x]);
        }
        else {
            cout << -(p - arr[x]) << endl;
