...
void solve() {
    ...
    for (int i = 1; i <= N; i++) psum[i] = psum[i - 1] + arr[i].second;
    ...
    for (int i = 1; i <= N; i++) {
        ...
        ll E = P;
        if (arr[i].first > E) continue;
        E += arr[i].second;
        int l = i - 1;
        int r = i + 1;
        
        while ((l > 0 && arr[l].first <= E) || (r <= N && arr[r].first <= E)) {
            if (l > 0 && arr[l].first <= E) {
                parent[findp(i)] = findp(l);
                E += psum[l] - psum[findp(l) - 1];
                l = findp(l) - 1;
            }
            if (r <= N && arr[r].first <= E) {
                parent[findp(r)] = findp(i);
                E += arr[r].second;
                r += 1;
            }
        }
