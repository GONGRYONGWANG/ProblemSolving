...
void solve(int tc) {
    ...
    while (l < r) {
        ll m = (l + r + 1) / 2;
        MF graph(2 * N + 3);
        int src = 2 * N + 1; int sink = 2 * N + 2;
        for (int i = 1; i <= N; i++) {
            graph.addEdge(src, i, arr[i]);
            if (B[i]) {
                graph.addEdge(i + N, sink, m);
        ...
        for (auto [u, v] : E) {
            if (B[u]) graph.addEdge(v, u + N, m);
            if (B[v]) graph.addEdge(u, v + N, m);
        }

        ll ret = graph.get(src, sink);
