...
int N, M;
int board[2000][2000];
int DP[2000][2000][4][2];

int dp(int x, int y, int t, bool b) {
    if (x >= N || y >= M) return 0;
    if (DP[x][y][t][b] != -1) return DP[x][y][t][b];
    int ret = max(dp(x + 1, y, t, b), dp(x, y + 1, t, b));
    if (!b) ret = max(ret, max(dp(x + 1, 0, t, 1), dp(0, y + 1, t, 1)));
    if (board[x][y] == t) ret += 1;
    return DP[x][y][t][b] = ret;
}
...
void solve() {
    ...
    m['Z'] = 0; m['O'] = 1; m['A'] = 2; m['C'] = 3;
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            for (int t = 0; t < 4; t++) DP[i][j][t][0] = DP[i][j][t][1] = -1;
            char x;
            cin >> x;
            board[i][j] = m[x];
        }
