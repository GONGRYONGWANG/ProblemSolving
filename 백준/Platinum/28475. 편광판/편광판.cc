...
struct treeNode {
    treeNode() {}
    treeNode(int l, int r, bool pass) : l(l), r(r), pass(pass) {}
    int l, r;
    bool pass;
};
...
treeNode mer(treeNode t1, treeNode t2) {
    if (t1.l == -1) return t2;
    if (t2.l == -1) return t1;
    bool p = (t1.pass & t2.pass);
    if (abs(t1.r - t2.l) % 2 == 0 && abs(t1.r - t2.l) % 4 != 0) p = false;
    return { t1.l, t2.r, p };
}

void makeTree(int node, int start, int end) {
    if (start == end) {
        tree[node] = { init[start],init[start],1 };
        return;
    }
    int mid = (start + end) / 2;
...
treeNode get(int node, int start, int end, int left, int right) {
    if (left > end || right < start) return { -1, 0, 1 };
