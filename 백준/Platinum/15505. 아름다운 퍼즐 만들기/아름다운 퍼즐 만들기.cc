...
void solve() {
    ...
    for (int hist = 0; hist < (1 << (2 * M)); hist++) {
        DP[hist].second = 1;
        int h = hist;
        int prv = -1;
        for (int j = 0; j < M; j++) {
            if (prv == h % 4) DP[hist].second = 0;
            prv = h % 4;
            DP[hist].first += arr[0][M - j - 1][h % 4];
            h /= 4;
        }
        if (DP[hist].second == 0) DP[hist].first = -INF;
    }
    ...
    for (int i = 1; i < N; i++) {
        for (int j = 0; j < M; j++) {
            vector<pll> nx((1 << (2 * M)), { -INF,0 });
            for (int hist = 0; hist < (1 << (2 * M)); hist++) {
                if (DP[hist].second == 0) continue;
                for (int k = 0; k < 4; k++) {
                    if (hist / (1 << (2 * M - 2)) == k) continue;
                    if (j != 0 && hist % 4 == k) continue;
                    int nxthist = hist % (1 << (2 * M - 2)) * 4 + k;
                    if (DP[hist].first + arr[i][j][k] > nx[nxthist].first) {
                        nx[nxthist] = { DP[hist].first + arr[i][j][k],0 };
                    }
                    if (nx[nxthist].first == DP[hist].first + arr[i][j][k]) {
                        nx[nxthist].second += DP[hist].second;
                        nx[nxthist].second %= mod;
                    }
                }
            }
            DP = nx;
    ...
    pll ans = { 0,0 };
    for (int hist = 0; hist < (1 << (2 * M)); hist++) {
        ll val = DP[hist].first;
        ll cnt = DP[hist].second;
        if (val > ans.first) {
            ans = { val,0 };
        }
        if (val == ans.first) {
