...
ll dp(int x) {
    ...
    if (arr[x + 1].first > arr[x].first + K) return DP[x] = ret;

    ll mx = arr[x + 1].second;
    
    for (int i = x + 2; i <= min(N - 1, x + 4); i++) {
        if (arr[i].first > arr[x].first + K) break;
        ret = max(ret, arr[x].second + mx + arr[i].second + dp(i + 1));
