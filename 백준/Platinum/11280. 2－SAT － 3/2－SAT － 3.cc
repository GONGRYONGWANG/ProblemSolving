...
struct TwoSAT {
    ...
    vector<vector<int>> adj_rev;   // 역방향 그래프
    vector<int> scc_id;            // 각 노드가 속한 SCC의 ID
    vector<int> visited;           // 방문 여부 확인
    stack<int> stk;                // DFS 스택
    int scc_count;                 // SCC의 개수

    // 생성자: 변수의 개수를 입력받아 초기화
    TwoSAT(int n) : n(n) {
        adj.resize(2 * n);
        adj_rev.resize(2 * n);
        scc_id.resize(2 * n, -1);
    ...
    // x 또는 y가 참이어야 하는 조건을 추가하는 함수
    void add_clause(int x, int y) {
        if (x < 0) x = neg(-x);
        if (y < 0) y = neg(-y);
        adj[neg(x)].push_back(y);
        adj[neg(y)].push_back(x);
        adj_rev[y].push_back(neg(x));
        adj_rev[x].push_back(neg(y));
    }

    // Tarjan 알고리즘을 사용해 SCC를 찾는 DFS 함수
    void dfs(int v, vector<vector<int>>& graph, vector<int>& order, int pass) {
        visited[v] = 1;
        for (int u : graph[v]) {
            if (!visited[u]) dfs(u, graph, order, pass);
        }
        if (pass == 1) {
            order.push_back(v);
        }
    ...
    bool solve() {
        vector<int> order;
        // 첫 번째 DFS 패스: 역방향 그래프에서 순서를 계산
        for (int i = 0; i < 2 * n; i++) {
            if (!visited[i]) dfs(i, adj_rev, order, 1);
        }

        // 두 번째 DFS 패스: 원래 그래프에서 SCC를 찾음
        ...
        // 변수와 그 반대의 변수가 같은 SCC에 있다면 불가능
        for (int i = 0; i < n; i++) {
            if (scc_id[i] == scc_id[neg(i)]) return false;
        }
        return true;
    }
};
