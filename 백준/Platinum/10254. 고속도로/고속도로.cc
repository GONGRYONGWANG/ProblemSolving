...
pair<int, int> rotatingCalipers(const vector<pair<int, int>>& hull) {
    int n = hull.size();
    if (n == 1) return { 0, 0 };
    if (n == 2) return { 0, 1 };
...
void solve(int tc) {
    ...
    vector<pii> CH = convexHull(arr);
    pii ret = rotatingCalipers(CH);

    cout << CH[ret.first].first << " " << CH[ret.first].second << " " << CH[ret.second].first << " " << CH[ret.second].second;
    cout << endl;
