...
void solve() {
    ...
    vector<vector<ll>> cycle = mat[0];
    for (int i = 1; i < T; i++) {
        cycle = matmul(cycle, mat[i], mod);
    }
    ...
    while (n) {
        if (n % 2) {
            ret = matmul(ret, cycle, mod);
        }
        n /= 2;
        cycle = matmul(cycle, cycle, mod);
    }

    D %= T;
    for (int i = 0; i < D; i++) {
        ret = matmul(ret, mat[i], mod);
    }
