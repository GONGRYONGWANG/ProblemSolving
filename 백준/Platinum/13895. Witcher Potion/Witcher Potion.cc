...
void solve(int tc) {
    ...
    vector<vector<int>> DP((1 << N), vector<int>(100, -1));
    DP[0][0] = 100;

    int ret = 0;
    while (1) {
        vector<vector<int>> nx((1 << N), vector<int>(100, -1));
        bool x = false;
        for (int hist = 0; hist < (1 << N); hist++) {
            for (int p = 0; p < 100; p++) {
                if (DP[hist][p] < 0) continue;
                for (int i = 0; i < N; i++) {
                    int a = arr[i].first; int b = arr[i].second;
                    if (hist & (1 << i)) continue;
                    if (DP[hist][p] + a <= K) continue;
                    if (p + b >= 100) continue;
                    nx[hist | (1 << i)][max(0, p + b - M)] = max(nx[hist | (1 << i)][max(0, p + b - M)], min(100, DP[hist][p] + a) - K);
                    x = true;
                }
                if (DP[hist][p] <= K) continue;
                nx[hist][max(0, p - M)] = max(nx[hist][max(0, p - M)], DP[hist][p] - K);
                x = true;
            }
        }
        if (x) ret += 1;
