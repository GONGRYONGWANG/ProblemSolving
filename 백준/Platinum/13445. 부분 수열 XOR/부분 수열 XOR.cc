...
struct trie {
    ll sz = 0;
    trie* nx[2] = { nullptr,nullptr };

    void update(int x, int idx) {
        sz += 1;
        if (idx == -1) return;
        if (nx[x / (1 << idx) % 2] == nullptr) nx[x / (1 << idx) % 2] = new trie;
        nx[x / (1 << idx) % 2]->update(x, idx - 1);
    }

    ll get(int x, int idx) {
        if (idx == -1) return 0;
        ll ret = 0;
        if (K / (1 << idx) % 2) {
            if (nx[x / (1 << idx) % 2] != nullptr) ret += nx[x / (1 << idx) % 2]->sz;
            if (nx[(x / (1 << idx) + 1) % 2] != nullptr) ret += nx[(x / (1 << idx) + 1) % 2]->get(x, idx - 1);
        }
        else if (nx[x / (1 << idx) % 2] != nullptr) ret += nx[x / (1 << idx) % 2]->get(x, idx - 1);
        return ret;
    }
