    ...
    while (!q.empty()) {
        ...
        if (!x) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (!visited[hist | (1 << i)][1]) {
                    q.push({ hist | (1 << i),1,d + arr[i] });
                }
                for (int x : v) {
                    if (arr[x] == arr[i]) continue;
                    if (visited[hist | (1 << i) | (1 << x)][1]) continue;
                    q.push({ hist | (1 << i) | (1 << x),1,d + arr[i] });
                }
                v.push_back(i);
            }
        }
            ...
            for (int i = 0; i < N; i++) {
                if (!(hist & (1 << i))) continue;
                if (!visited[hist - (1 << i)][0]) {
                    q.push({ hist - (1 << i),0,d + arr[i] });
                }
                for (int x : v) {
                    if (arr[x] == arr[i]) continue;
                    if (visited[hist - (1 << i) - (1 << x)][0]) continue;
                    q.push({ hist - (1 << i) - (1 << x),0,d + arr[i] });
                }
                v.push_back(i);
            }
        }
