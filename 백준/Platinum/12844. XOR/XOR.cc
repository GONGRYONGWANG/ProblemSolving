...
int get(int l, int r){
    int ret= 0;
    __m256i xor_value = _mm256_set1_epi32(0);
    ...
    for(; i+8<=r;i+=8){
        __m256i data = _mm256_loadu_si256((__m256i*)&arr[i]);
        __m256i result = _mm256_xor_si256(data, xor_value);
        _mm256_storeu_si256((__m256i*)&xor_value, result);
    }
    for(; i<=r;i++){
        ret^=arr[i];
    }
    ...
    for(int i=0; i<8;i++){
        ret ^= _mm256_extract_epi32(xor_value, i);
    }

    return ret;
}

int getbck(int lbck, int rbck){
    int ret= 0;
    __m256i xor_value = _mm256_set1_epi32(0);
    ...
    for(; i+8<=rbck;i+=8){
        __m256i data = _mm256_loadu_si256((__m256i*)&bucket[i]);
        __m256i result = _mm256_xor_si256(data, xor_value);
        _mm256_storeu_si256((__m256i*)&xor_value, result);
    }
    for(; i<=rbck;i++){
...
void solve() {
    ...
    while(Q--){
        ...
        if(t==1){
            ...
            if(lbck ==rbck){
                update(l,r,x);
                if((r-l+1)%2) bucket[lbck]^=x;
                continue;
            }
            update(l,(lbck+1)*704-1,x);
            if(((lbck+1)*704 - l)%2) bucket[lbck]^=x;
            updatebck(lbck+1 , rbck-1 , x);
            update(rbck*704,r,x);
            if((r - rbck*704 + 1)%2) bucket[rbck]^=x;
        }
        else{
            int l,r;
            cin>>l>>r;
            ...
            cout<< (get(l,(lbck+1)*704-1)^get(rbck*704,r) ^ getbck(lbck+1,rbck-1))<<endl;
        }
    }
