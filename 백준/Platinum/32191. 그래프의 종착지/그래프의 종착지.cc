...
void solve() {
    ...
    for (int i = 1; i <= N; i++) {
        ...
        cin >> x;
        if (x == -1) continue;
        while (child[i].front() != x) {
            child[i].push_back(child[i].front());
            child[i].pop_front();
        }
    }
    ...
    while (!q.empty()) {
        int cur = q.front();
        q.pop();
        if (child[cur].size() == 0) continue;
        int d = sz[cur] / child[cur].size();
        for (int i = 0; i < child[cur].size(); i++) {
            int x = child[cur][i];
            sz[x] += d;
            if (i < sz[cur] % child[cur].size()) sz[x] += 1;
            pcnt[x] -= 1;
            if (pcnt[x] == 0) q.push(x);
            if (ans[cur] && i  == (sz[cur]+(child[cur].size()-1)) % child[cur].size()) ans[x] = true;
        }
    }

    for (int i = 1; i <= N; i++) {
        if (ans[i] && child[i].size() == 0) cout << i;
    }
