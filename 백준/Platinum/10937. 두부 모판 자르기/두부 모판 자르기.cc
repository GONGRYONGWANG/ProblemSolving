...
int board[11][11];
int score[4][4] = { {100,70,40,0},{70,50,30,0},{40,30,20,0},{0,0,0,0} };
...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            cin >> x;
            if (x == 'A') board[i][j] = 0;
            else if (x == 'B') board[i][j] = 1;
            else if (x == 'C') board[i][j] = 2;
            else board[i][j] = 3;
        }
    }
    vector<vector<int>> E;
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            for (int dir = 0; dir < 4; dir++) {
                ...
                int u = i * N + j; int v = nx * N + ny;
                int w = -score[board[i][j]][board[nx][ny]];
                E.push_back({ u,v,w });
            }
        }
    }
    ...
    for (int flow = 1; flow <= N * N / 2; flow++) {
        MCMF graph(N * N + 3);
        int src = N * N; int sink = N * N + 1; int sink2 = N * N + 2;
        graph.addEdge(sink, sink2, flow, 0);
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                if ((i + j) % 2) {
                    int  v = i * N + j;
                    graph.addEdge(v, sink, 1, 0);
                }
                else {
                    int u = i * N + j;
                    graph.addEdge(src, u, 1, 0);
                }
            }
        }
        for (vector<int>& e : E) {
        ...
        ans = max(ans, -graph.get(src, sink2).second);
    }
