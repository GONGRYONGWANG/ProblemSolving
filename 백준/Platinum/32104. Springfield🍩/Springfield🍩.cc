    ...
    while (Q--) {
        ...
        if (t == 1) {
            ...
            for (auto iter = m[b].begin(); iter != m[b].end(); iter++) {
                ...
                int s = l; int e = r;
                auto it = m2[a].lower_bound(l);
                if (m2[a].lower_bound(l) != m2[a].end()) {
                    s = min(l, m2[a].lower_bound(l)->second);
                }
                it = m[a].upper_bound(r);
                if (it != m[a].begin()) {
                    it--;
                    e = max(e, it->second);
                }

                it = m[a].lower_bound(s);
                while (it != m[a].end() && it->second <= e) {
                    sz[a] -= (it->second) - (it->first) + 1;
                    auto nit = it;
                    nit++;
                    m[a].erase(it);
                    it = nit;
                }
                it = m2[a].lower_bound(s);
                while (it != m2[a].end() && it->first <= e) {
                    auto nit = it;
                    nit++;
                    m2[a].erase(it);
                    it = nit;
