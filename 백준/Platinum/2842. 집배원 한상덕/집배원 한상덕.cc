...
void solve() {
    ...
    for (int mnidx = 0; mnidx < arr.size(); mnidx++) {

        int mn = arr[mnidx];

        if (h[s.first][s.second] < mn) break;

        int tmp = 1;
        bool b = false;
        while (tmp--) {
            int mxidx = arr.size()-1;
            int mx = arr[mxidx];
            int cnt = 0;
            ...
            while (!q.empty()) {
                ...
                q.pop();
                if (board[x][y] == 'K') cnt += 1;
                for (int dir = 0; dir < 8; dir++) {
                    int nx = x + dx[dir];
                    int ny = y + dy[dir];
                    if (nx < 0 || nx >= N || ny < 0 || ny >= N) continue;
                    if (visited[nx][ny]) continue;
                    if (h[nx][ny]<mn || h[nx][ny]>mx) continue;
                    visited[nx][ny] = true;
                    q.push({ nx,ny });
                }
            }
        ...
        int l = mnidx; int r = arr.size() - 1;

        while (l < r) {
            int mxidx = (l + r) / 2;
            int mx = arr[mxidx];
            if (h[s.first][s.second] > mx) {
                l = mxidx + 1;
                continue;
            }
            int cnt = 0;
