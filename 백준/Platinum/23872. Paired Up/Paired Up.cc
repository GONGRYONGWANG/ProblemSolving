...
vector<int> W;
vector<int> nx;
int DP[100000][2]; // 0:연결, 1:연결x
int dp(int x, bool b) {
    if (x == N) return 0;
    if (DP[x][b] != -inf - 1) return DP[x][b];
    DP[x][b] = -inf;
    int k = nx[x];
    if (!b) {
        if (k == x + 1) return DP[x][b] = -inf;
        DP[x][b] = max(DP[x][b], max(dp(x + 2, 0), dp(x + 2, 1))); // x,x+1 연결
        if (k > x + 2) { // x,x+2연결  x+1 연결 안함
            int nk = nx[x + 1];
            if ((nk - (x + 1) - 1 - 1) % 2) DP[x][b] = max(DP[x][b], W[x + 1] + dp(nk - 1, 0));
            else DP[x][b] = max(DP[x][b], W[x + 1] + max(dp(nk, 0), dp(nk, 1)));
        }
        return DP[x][b];
    }

    if ((k - x - 1) % 2) DP[x][b] = max(DP[x][b], W[x] + dp(k - 1, 0));
    else DP[x][b] = max(DP[x][b], W[x] + max(dp(k, 0), dp(k, 1)));

    return DP[x][b];
}

int dp2(int x, bool b) {
    if (x == N) return 0;
    if (DP[x][b] != -inf - 1) return DP[x][b];
    DP[x][b] = inf;
    int k = nx[x];
    if (!b) {
        if (k == x + 1) return DP[x][b] = inf;
        DP[x][b] = min(DP[x][b], min(dp2(x + 2, 0), dp2(x + 2, 1))); // x,x+1 연결
        if (k > x + 2) { // x,x+2연결  x+1 연결 안함
            int nk = nx[x + 1];
            if ((nk - (x + 1) - 1 - 1) % 2) DP[x][b] = min(DP[x][b], W[x + 1] + dp2(nk - 1, 0));
            else DP[x][b] = min(DP[x][b], W[x + 1] + min(dp2(nk, 0), dp2(nk, 1)));
        }
        return DP[x][b];
    }

    if ((k - x - 1) % 2) DP[x][b] = min(DP[x][b], W[x] + dp2(k - 1, 0));
    else DP[x][b] = min(DP[x][b], W[x] + min(dp2(k, 0), dp2(k, 1)));
...
void solve(int tc) {
    ...
    if (T == 2) cout << max(dp(0, 0), dp(0, 1));
    else cout << min(dp2(0, 0), dp2(0, 1));
