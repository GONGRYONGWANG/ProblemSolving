...
void solve() {
    ...
    for (int i = 1; i <= V; i++) {
        if (!m.count(graph.scc_id[i])) cnt += 1;
        m[graph.scc_id[i]].push_back(i);
    ...
    for (int i = 1; i <= V; i++) {
        if (visited.count(graph.scc_id[i])) continue;
        for (int x : m[graph.scc_id[i]]) {
            cout << x << " ";
        }
        cout << -1 << endl;
        visited.insert(graph.scc_id[i]);
    }
