...
struct treeNode {
    treeNode() {}
    treeNode(int mn, int mx,int val) : mn(mn), mx(mx), val(val) {}
    int mn, mx, val;
};

treeNode mer(const treeNode& A, const treeNode& B) {
    return { min(A.mn, B.mn) , max(A.mx, B.mx), max(max(A.val, B.val), B.mx - A.mn) };
}
...
void update(int node, int start, int end, int k, int x) {
    if (k > end || k < start) return;
    if (start == end) {
        tree[node].mx = tree[node].mn = x;
        return;
    }
    int mid = (start + end) / 2;
    update(node * 2, start, mid, k, x);
    update(node * 2 + 1, mid + 1, end, k, x);
    tree[node] = mer(tree[node * 2], tree[node * 2 + 1]);
}

treeNode get(int node, int start, int end, int left, int right) {
    if (left > end || right < start) return { inf,-inf,0 };
