...
void solve() {
    ...
    while (!q.empty()) {
        ...
        if (!visited[x][y][idx + 1]) {
            visited[x][y][idx + 1] = true;
            q.push({ x,y,idx + 1 });
        }

        if (x + arr[idx] < (total + 1) / 2 && !visited[x+arr[idx]][y][idx+1]) {
            visited[x + arr[idx]][y][idx + 1] = true;
            q.push({ x + arr[idx], y,idx + 1 });
        }

        if (y + arr[idx] < (total + 1) / 2 && !visited[x][y + arr[idx]][idx + 1]) {
            visited[x][y + arr[idx]][idx + 1] = true;
            q.push({ x,y + arr[idx], idx + 1 });
        }

    }

    ll ans = -1;
    for (ll x = 1; x < (total + 1) / 2; x++) {
        for (ll y = 1; y < (total + 1) / 2; y++) {
            ll z = total - x - y;
            if (z >= (total + 1) / 2) continue;
            if (!visit[x][y]) continue;
            ll ret = sqrt(total * (total - 2 * x) * (total - 2 * y) * (total - 2 * z) * 625);
            ans = max(ans, ret);
        }
    }
