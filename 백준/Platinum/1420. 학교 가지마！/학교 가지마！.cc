...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            cin >> x;
            if(x=='#') continue;
            if (x != 'K')flow.addEdge(i * M + j, i * M + j + N * M, 1);
            else flow.addEdge(i * M + j, i * M + j + N * M, INF);

            for (int dir = 0; dir < 4; dir++) {
                int nx = i + dx[dir];
                int ny = j + dy[dir];
                if (nx < 0 || nx >= N || ny < 0 || ny >= M) continue;
                flow.addEdge(i * M + j + N * M, nx * M + ny, 1);
            }
    ...
    if (src / M == sink / M && abs(src % M - sink % M) == 1) {
        cout << -1;
        return;
    }
    if (src % M == sink % M && abs(src / M - sink / M) == 1) {
