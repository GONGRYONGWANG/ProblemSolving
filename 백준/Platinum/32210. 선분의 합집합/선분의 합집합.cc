...
void solve() {
    ...
    vector<bool> ans(Q);
    for (int i = 0; i < Q; i++) {
        int a = query[i][1]; int b = query[i][0]; 
        while (idx < N && arr[idx].first <= b) {
            int p = arr[idx].second; int l = arr[idx].first;
            idx += 1;
            for (int j = 1000000 - p; j >= 0; j--) {
                if (mxlen[j] == -1) continue;
                mxlen[j + p] = max(mxlen[j + p], mxlen[j] + l);
            }
        }

        ans[query[i][2]] = (mxlen[a] >= b);
    }
