...
void solve() {
    ...
    for (int i = 1; i < 17; i++) {
        for (int j = 1; j <= N; j++) {
            ...
            DP[j][i].second = DP[j][i - 1].second | DP[DP[j][i - 1].first][i - 1].second;
        }
    }
    ...
    for (int i = 1; i <= N; i++) {
        if (arr[i] == candy) candy = 0;
    }

    if (candy == 0) cout << "PLAY";
    else cout << "CRY";
    cout << endl;
    ...
    while (M--) {
        ...
        for (int i = 16; i >= 0; i--) {
            if (DP[u][i].first != DP[v][i].first) {
                ret |= DP[u][i].second; ret |= DP[v][i].second;
                u = DP[u][i].first; v = DP[v][i].first;
            }
        }
