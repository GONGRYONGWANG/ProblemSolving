...
void solve() {
    ...
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= N; j++) {
            if (j == 1 || board[i][j - 1] == '#') cnt1 += 1;
            if (board[i][j] == '*') comp[i][j].first = cnt1;
        }
    }
    ...
    for (int j = 1; j <= M; j++) {
        for (int i = 1; i <= N; i++) {
            if (i == 1 || board[i-1][j] == '#') cnt2 += 1;
            if (board[i][j] == '*') comp[i][j].second = cnt2;
        }
    }

    BiMatch graph(cnt1, cnt2);

    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= M; j++) {
            if (board[i][j] == '*') {
                graph.addEdge(comp[i][j].first, comp[i][j].second);
            }
        }
    }
