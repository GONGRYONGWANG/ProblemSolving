...
void dfs(int cur, int p) {
    ...
    if (heavychild == 0) {
        m[cur] = new map<int, ll>;
    }
    else {
        m[cur] = m[heavychild];
        ans[cur] = ans[heavychild];
    }

    for (auto it = (*init[cur]).begin(); it != (*init[cur]).end(); it++) {
        ans[cur] -= (*m[cur])[it->first] * (total[it->first] - (*m[cur])[it->first]);
        (*m[cur])[it->first] += it->second;
        ans[cur] += (*m[cur])[it->first] * (total[it->first] - (*m[cur])[it->first]);
    }

    for (int nx : E[cur]) {
        if (nx == heavychild || nx == p) continue;
