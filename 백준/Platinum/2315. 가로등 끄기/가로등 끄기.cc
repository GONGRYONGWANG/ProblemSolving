...
ll dp(int cur, int p) {
    ...
    ll ret = INF;
    ll w = arr[N].second - arr[r].second + arr[l - 1].second;
    if (l != 1) ret = min(ret, dp(l - 1, r) + (arr[cur].first - arr[l - 1].first) * w);
    if (r != N) ret = min(ret, dp(r + 1, l) + (arr[r + 1].first - arr[cur].first) * w);
    return DP[cur][p] = ret;
}
