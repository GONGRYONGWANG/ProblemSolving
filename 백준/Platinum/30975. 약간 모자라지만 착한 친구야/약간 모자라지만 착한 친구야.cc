...
int dp(int cur, int hist) {
    if (DP[cur][hist]) return DP[cur][hist];
    if (hist == (1 << N) - 1) {
        if (!dist[cur][N]) return inf;
        return dist[cur][N];
    }
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (prv[i] != i && !(hist & (1 << prv[i]))) continue;
        ret = min(ret, dist[cur][i] + dp(i, hist | (1 << i)));
    }
    return DP[cur][hist] = ret;
}
...
void solve() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        if (dist[u][v]) dist[u][v] = min(dist[u][v], w);
        else dist[u][v] = w;
