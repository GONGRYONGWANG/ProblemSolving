...
int get(int x, int flow) {
    while (cap[parent[x]] <= flow) {
        x = parent[x];
    }

    if (cap[x] == flow) return sz[x];

    if (cap[x] < flow) {
        int p = parent[x];

        if (child[p].size() == 1) return sz[x];

        int sibling;
        if (child[p][0] == x) sibling = child[p][1];
        else sibling = child[p][0];

        return sz[x] + get(sibling, min(flow - cap[x], cap[sibling]));
    }

    // cap[x]>flow

    if (child[x].size() == 0) return 0;
    if (child[x].size() == 1) return get(child[x][0], min(flow, cap[child[x][0]]));


    if (cap[child[x][0]] <= flow / 2) return sz[child[x][0]] + get(child[x][1], min(flow - cap[child[x][0]], cap[child[x][1]]));
    if (cap[child[x][1]] <= flow / 2) return sz[child[x][1]] + get(child[x][0], min(flow - cap[child[x][1]], cap[child[x][0]]));

    return get(child[x][0], flow / 2) + get(child[x][1], flow / 2);
