...
void solve(int tc) {
    ...
    vector<vector<ll>> DP(H, vector<ll>(X + 1, 0)); // 0:empty, 1: soloI, 2: notsoloI
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            vector<vector<ll>> nx(H, vector<ll>(X + 1, 0));

            for (int hist = 0; hist < H; hist++) {

                if (hist / (H / 3) != 1) {
                    for (int k = 0; k <= X; k++) {
                        nx[hist * 3 % H + 0][k] += DP[hist][k];
                        nx[hist * 3 % H + 0][k] %= mod;
                    }
                }
                ...
                if (hist / (H / 3) != 0 || (j != 0 && hist % 3 != 0)) {
                    for (int k = 0; k < X; k++) {
                        int nxhist = hist * 3 % H + 2;
                        if (j != 0 && hist % 3 == 1) nxhist += 3;
                        nx[nxhist][k + 1] += DP[hist][k];
                        nx[nxhist][k + 1] %= mod;
                    }
                }
                else {
                    for (int k = 0; k < X; k++) {
                        nx[hist * 3 % H + 1][k + 1] += DP[hist][k];
                        nx[hist * 3 % H + 1][k + 1] %= mod;
                    }
                }
            }
            DP = nx;
