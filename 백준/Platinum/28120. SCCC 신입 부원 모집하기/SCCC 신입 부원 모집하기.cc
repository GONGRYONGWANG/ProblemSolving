...
void solve(int tc) {
    ...
    MCMF graph(N + K + 3);
    int src = N + K + 1, sink = N + K + 2;

    for (int i = N + 1; i <= N + K; i++) {
        graph.addEdge(i, sink, X, 0);
    }
    ...
    for (int k = 1; k <= K; k++) {
        vector<int> ret;
        for (int i = 0; i < graph.adj[N + k].size(); i++) {
            if (graph.adj[N + k][i].to == sink) continue;
            if (graph.adj[N + k][i].flow != -1) continue;
            ret.push_back(graph.adj[N + k][i].to);
        }
        cout << ret.size() << " ";
