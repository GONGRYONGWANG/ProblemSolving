...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        cin >> x;
        int ret = mx[x % K] + 1;
        ret = max(ret, get(1, 1, 500000, max(1, x - D), min(500000, x + D)) + 1);
        mx[x % K] = max(mx[x % K], ret);
        if (ret > get(1, 1, 500000, x, x)) {
            update(1, 1, 500000, x, ret);
        }
    }
    cout << get(1, 1, 500000, 1, 500000);
}
