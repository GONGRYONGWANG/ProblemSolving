...
void solve(int tc) {
    ...
    MF graph(2 * N * N + 2);
    int src = 2 * N * N; int sink = 2 * N * N + 1;

    int total = 0;

    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            int in = i * N + j; int out = in + N * N;
            if ((i + j) % 2) graph.addEdge(src, in, inf);
            else graph.addEdge(out, sink, inf);
            int x;
            cin >> x;
            total += x;
            graph.addEdge(in, out, x);
            if ((i + j) % 2) {
                for (int dir = 0; dir < 8; dir++) {
                    int nx = i + dx[dir]; int ny = j + dy[dir];
                    if (nx < 0 || nx >= N || ny < 0 || ny >= N) continue;
                    graph.addEdge(out, nx * N + ny, inf);
                }
            }
        }
    }

    cout << total - graph.get(src, sink);
