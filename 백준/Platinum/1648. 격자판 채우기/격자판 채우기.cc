...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            for (int hist = 0; hist < (1 << M); hist++) {
                if (hist / (1 << (M - 1)) == 0) {
                    nxt[hist % (1 << (M - 1)) * 2 + 1] += DP[hist];
                    nxt[hist % (1 << (M - 1)) * 2 + 1] %= mod;
                }
                else {
                    nxt[hist % (1 << (M - 1)) * 2] += DP[hist];
                    nxt[hist % (1 << (M - 1)) * 2] %= mod;
                    if (j != 0 && hist % 2 == 0) {
                        nxt[(hist + 1) % (1 << (M - 1)) * 2 + 1] += DP[hist];
                        nxt[(hist + 1) % (1 << (M - 1)) * 2 + 1] %= mod;
                    }
                }
            }
            for (int hist = 0; hist < (1 << M); hist++) {
