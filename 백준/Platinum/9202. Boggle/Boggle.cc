...
void bt(int x, int y, string s) {
    s.push_back(board[x][y]);
    if (!t.find(s)) return;
    visited[x][y] = true;
    if (m.count(s) && !check[m[s]]) {
        check[m[s]] = true;
        cnt += 1;
        ans += score[s.length()];
        if (s.length() > ansS.length() || (s.length() == ansS.length() && s < ansS)) {
            ansS = s;
        }
    }
