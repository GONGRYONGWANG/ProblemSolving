...
void updatelen(int idx) {
    if (idx < 0) return;
    sum[idx / 316] -= len[idx];
    len[idx] = abs(arr[idx].first - arr[idx + 1].first) + abs(arr[idx].second - arr[idx + 1].second);
    sum[idx / 316] += len[idx];
}

void updated(int idx) {
    if (idx <= 0 || idx >= N - 1) return;
    d[idx] = len[idx - 1] + len[idx] - abs(arr[idx - 1].first - arr[idx + 1].first) - abs(arr[idx - 1].second - arr[idx + 1].second);
    int bck = idx / 316;
    mx[bck] = 0;
    for (int i = max(1, bck * 316); i < min((bck + 1) * 316, N - 1); i++) {
        mx[bck] = max(mx[bck], d[i]);
    }
}

int getsum(int l, int r) {
    if (r < l) return 0;
    int lbck = l / 316; int rbck = r / 316;
    int ret = 0;
    if (lbck == rbck) {
        for (int i = l; i <= r; i++) {
            ret += len[i];
    ...
    for (int i = l; i < (lbck + 1) * 316; i++) {
        ret += len[i];
    }
    for (int i = rbck * 316; i <= r; i++) {
        ret += len[i];
    }
    for (int i = lbck + 1; i < rbck; i++) {
        ret += sum[i];
    }
    return ret;
}
...
void solve() {
    ...
    for (int i = 0; i < N - 1; i++) {
        len[i] = abs(arr[i].first - arr[i + 1].first) + abs(arr[i].second - arr[i + 1].second);
        sum[i / 316] += len[i];
    }
    for (int i = 1; i < N - 1; i++) {
        d[i] = len[i - 1] + len[i] - abs(arr[i - 1].first - arr[i + 1].first) - abs(arr[i - 1].second - arr[i + 1].second);
        mx[i / 316] = max(mx[i / 316], d[i]);
    }
    ...
    while (Q--) {
        ...
        if (t == 'U') {
            ...
            I -= 1;
            arr[I] = { x,y };
            updatelen(I); updatelen(I - 1);
            updated(I - 1); updated(I); updated(I + 1);
        }
        else {
            int l, r;
            cin >> l >> r;
            l -= 1; r -= 1;
            cout << getsum(l, r - 1) - getmx(l + 1, r - 1) << endl;
        }
    }
