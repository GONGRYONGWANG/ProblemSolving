...
void solve(int tc) {
    ...
    ll score = 0;
    while (aidx != K && A[aidx].first < B[0]) {
        score += A[aidx].second;
        aidx += 1;
    }
    v.push_back({ score,0 });

    for (int bidx = 0; bidx < M - 1; bidx++) {
        ll total = 0;
        ll mx = 0;

        vector<pll> tmp;
        while (aidx != K && A[aidx].first < B[bidx + 1]) {
            total += A[aidx].second;
            tmp.push_back(A[aidx]);
            aidx += 1;
        }
        ...
        for (int l = 0; l < tmp.size(); l++) {
            ll point = min(tmp[l].first + (tmp[l].first - B[bidx]), B[bidx + 1]);
            while (r != tmp.size() && (tmp[r].first <= point || tmp[r].first - point < B[bidx + 1] - tmp[r].first)) {
                ret += tmp[r].second;
                r += 1;
            }
            mx = max(mx, ret);
    ...
    while (N-- && !q.empty()) {
        ...
        bool b = q.top()[1];
        q.pop();
        if (!b) q.push({ idx,1,v[idx].second });
    }
