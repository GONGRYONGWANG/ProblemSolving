...
void solve() {
    ...
    for (int i = 0; i < N - 1; i++) {
        ...
        int x2 = arr[i + 1][0]; int y2 = arr[i + 1][1];
        pii ret = { abs(x1 - x2), abs(y1 - y2) };
        int g = gcd(ret.first, ret.second);
        ret.first /= g; ret.second /= g;

        int u = arr[i][2]; int v = arr[i + 1][2];
        if (u > v) swap(u, v);

        if (ret == mx) {
            ans = min(ans, { u,v });
        }
        else if (larger(mx, ret)) {
            ans = { u,v };
            mx = ret;
        }
    }
    ...
    for (int i = 0; i < N; i++) {
        if (arr[i][2] <= u) continue;
        if (arr[i][2] >= ans.second) continue;
        int x1 = arr[x][0]; int y1 = arr[x][1];
