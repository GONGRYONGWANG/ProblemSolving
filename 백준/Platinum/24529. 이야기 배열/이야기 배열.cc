...
int DP[50][51][51][51][3];

int dp(int n, int a, int b, int c, int prv) {

    if (n == N) return 0;
    if (a == N && b == N && c == N) return -inf;
    if (DP[n][a][b][c][prv]) return DP[n][a][b][c][prv];

    int ret = -inf;

    if (a != N) ret = max(ret, dp(n, a + 1, b, c, prv));
    if (b != N) ret = max(ret, dp(n, a, b + 1, c, prv));
    if (c != N) ret = max(ret, dp(n, a, b, c + 1, prv));

    if (prv != 0 && a != N && arr[0][a].first <= D[n]) ret = max(ret, arr[0][a].second + dp(n + 1, a + 1, b, c, 0));
    if (prv != 1 && b != N && arr[1][b].first <= D[n]) ret = max(ret, arr[1][b].second + dp(n + 1, a, b + 1, c, 1));
    if (prv != 2 && c != N && arr[2][c].first <= D[n]) ret = max(ret, arr[2][c].second + dp(n + 1, a, b, c + 1, 2));

    return DP[n][a][b][c][prv] = ret;
...
void solve() {
    ...
    for (int k = 0; k < 3; k++) {
        arr[k].resize(N);
        for (int i = 0; i < N; i++) {
            cin >> arr[k][i].second >> arr[k][i].first;
        }
    ...
    int ans =  max(dp(0, 0, 0, 0, 0), dp(0, 0, 0, 0, 1));
    if (ans < 0) cout << -1;
    else cout << ans;
