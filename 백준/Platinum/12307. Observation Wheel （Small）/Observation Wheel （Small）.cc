...
long double dp(int N , int hist) {
    if (hist == (1 << N) - 1) return 0;
    if (DP[N][hist]) return DP[N][hist];
    if (hist & (1 << (N - 1))) return DP[N][hist] = dp(N, ((hist % (1 << (N - 1))) << 1) | 1);
    long double ret = 0;
    long double x = 0;
    for (int i = 0; i < N; i++) {
        x += 1;
        if (hist & (1 << i)) continue;
        ret += (dp(N, hist | (1 << i)) + (2 * N - x + 1) / 2) * x / N;
        x = 0;
    }
    return DP[N][hist] = ret;
}
