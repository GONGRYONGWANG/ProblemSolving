...
struct MF { // Dinic O(V^2*E)
    ...
    void trace(int N, int src) {
        ...
        for (int i = 1; i <= N; i++) {
            if (visited[i] && !visited[adj[i][0].to]) cout << i << " ";
        }
...
void solve() {
    ...
    for (int i = 1; i <= N; i++) {
        int x;
        cin >> x;
        flow.addEdge(i, N + M + i, x);
    }
    for (int i = 0; i < M; i++) {
        int u, v;
        cin >> u >> v;
        flow.addEdge(N + M + u, v, INF);
        flow.addEdge(N + M + v, u, INF);
    }

    if (flow.get(src, sink) == 0) {
        cout << endl;
        return;
    }
