...
short N, P, D;
pair<short,short> arr[10];
pair<short,short> DP[10][(1 << 20)];
int pow4[10];

pii dp(int y, int hist) {
    int x = -1;
    for (int i = 0; i < N; i++) {
        if (hist / pow4[i] % 4 < P) {
            x = i;
    ...
    if (y == N) {
        hist -= hist / pow4[x] % 4 * pow4[x];
        hist += pow4[x] * P;
        return dp(x + 2, hist);
    }

    if (DP[y][hist].first != -1) return DP[y][hist];

    pair<short, short> ret1 = dp(y + 1, hist);

    if (x >= y) return DP[y][hist] = ret1;

    if (hist / pow4[y] % 4 == P) return DP[y][hist] = ret1;
    
    short dist = abs(arr[x].first - arr[y].first) + abs(arr[x].second - arr[y].second);
    if (dist < D) return DP[y][hist] = ret1;

    pair<short, short> ret2 = dp(y + 1, hist + pow4[x] + pow4[y]);
    if (hist / pow4[x] % 4 == P - 1) {
        ret2 = dp(0, hist + pow4[x] + pow4[y]);
    }
    ret2.first += 1; ret2.second += dist;

    if (ret2.first > ret1.first) return DP[y][hist] = ret2;
    else if (ret2.first == ret1.first && ret2.second < ret1.second) return DP[y][hist] = ret2;
    else return DP[y][hist] = ret1;
...
void solve() {
    ...
    cin >> P >> D;
    pow4[0] = 1;
    for (int i = 1; i < N; i++) pow4[i] = pow4[i - 1] * 4;
