...
void solve() {
    ...
    while (!q.empty()) {
        ...
        int d = q.top().second;
        q.pop();
        if (distt[x] != -1) continue;
        distt[x] = d;
        for (pii& e : E[x]) {
            if (distt[e.first] != -1) continue;
            q.push({ e.first,e.second + d });
        }
    }
    ...
    for (int i = 1; i <= N; i++) {
        for (pii& e : E[i]) {
            int u = i; int v = e.first;
            ans = max(ans, total - min(dists[u] + distt[v], dists[v] + distt[u]));
        }
    }
