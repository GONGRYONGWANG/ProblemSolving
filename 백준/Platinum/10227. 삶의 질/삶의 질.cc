...
void solve(int tc) {
    ...
    while (l < r) {
        ...
        for (int i = 1; i <= R; i++) {
            for (int j = 1; j <= C; j++) {
                rsum[i][j] = rsum[i][j - 1] + (board[i][j] <= m);
                csum[i][j] = csum[i - 1][j] + (board[i][j] <= m);
            }
        }
        ...
        for (int i = 1; i <= R - H + 1; i++) {
            if (i != 1) ret[i][1] = ret[i - 1][1] - rsum[i - 1][W] + rsum[i + H - 1][W];
            for (int j = 2; j <= C - W + 1; j++) {
                ret[i][j] = ret[i][j - 1] - (csum[i + H - 1][j - 1] - csum[i - 1][j - 1]) + (csum[i + H - 1][j + W - 1] - csum[i - 1][j + W - 1]);
            }
        }
