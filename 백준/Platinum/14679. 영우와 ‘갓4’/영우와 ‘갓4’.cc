...
void solve() {
    ...
    for (int i = 1; i <= 100; i++) {
        nA[i] = (powmod(i, Ap, 100) + Aa) % 100 + 1;
    }
    for (int i = 1; i <= 4; i++) {
        nD[i]= (powmod(i, Dp, 3) + Da) % 3 + 1;
    }
    for (int i = 1; i <= 1000; i++) {
        nH[i] = (powmod(i, Hp, 1000) + Ha) % 1000 + 1;
    }

    while (N--) {
        ll Dmg = max(1ll, A - mD);
        ll mDmg = max(1ll, mA - D);
        ll T = mH / Dmg + (mH % Dmg != 0);
        ll mT = H / mDmg + (H % mDmg != 0);
        if (T > mT) {
            cout << -1;
            return;
        }
        A += mA; D += mD; H += mH;
        mA = nA[mA]; mD = nD[mD]; mH = nH[mH];
    }
