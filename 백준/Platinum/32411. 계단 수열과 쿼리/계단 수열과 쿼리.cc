...
Node mer(Node A, Node B) {
    ...
    for (int i = 1; i <= 10; i++) {
        if (A.left[i] == A.sz && abs(A.rightval - B.leftval) == i) ret.left[i] = A.sz + B.left[i];
        else ret.left[i] = A.left[i];

        if (B.right[i] == B.sz && abs(A.rightval - B.leftval) == i) ret.right[i] = B.sz + A.right[i];
        else ret.right[i] = B.right[i];

        ret.mx[i] = max(max(ret.left[i], ret.right[i]), max(A.mx[i], B.mx[i]));
        if(abs(A.rightval - B.leftval) == i) ret.mx[i] = max(ret.mx[i], A.right[i] + B.left[i]);
    }
    return ret;
}
...
void lazyProp(int node, int start, int end) {
    ...
    tree[node].leftval += lazy[node];
    tree[node].rightval += lazy[node];
    if (start != end) {
