...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        cin >> x >> y;
        x += 1000;
        arr[x].first = min(arr[x].first, y);
        arr[x].second = max(arr[x].second, y);
    }
    ...
    for (int i = 0; i <= 2000; i++) {
        for (int j = 0; j <= 2000; j++) {
            if (arr[i].first == inf || arr[j].second == -inf) continue;
            long double ret = sqrt((i - j) * (i - j) + (arr[i].first - arr[j].second) * (arr[i].first - arr[j].second));
            ans = max(ans, ret);
        }
    }
