    ...
    while (1) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                char x;
                cin >> x;
                if (x == 'm') child.push_back({ i,j });
                else if (x == 'H') home.push_back({ i,j });
            }
        }
        ...
        MCMF graph(2*sz + 2);
        int src = sz * 2; int sink = sz * 2 + 1;
        for (int i = 0; i < sz; i++) {
            graph.addEdge(src, i, 1, 0);
            graph.addEdge(sz + i, sink, 1, 0);
        }
        for (int i = 0; i < sz; i++) {
            for (int j = 0; j < sz; j++) {
                auto [x1, y1] = child[i];
                ...
                graph.addEdge(i, j + sz, 1, dist);
            }
        }

        cout << graph.get(src, sink).second << endl;
