    ...
    for (int x = 0; x < N; x++) {
        for (int y = 0; y < M; y++) {
            ...
            for (int dir = 0; dir < 8; dir++) {
                int nx, ny, nnx, nny;
                nx = x + dx[dir]; nnx = nx + dx[dir];
                ny = y + dy[dir]; nny = ny + dy[dir];
                if (nnx<0 || nnx>=N || nny < 0 || nny >= M) continue;
                if (board[nx][ny] == 'M' || board[nnx][nny] == 'M' || board[nx][ny] == board[nnx][nny]) continue;
                if (board[nx][ny] == 'S') graph.addEdge(nx * M + ny + 1, nnx * M + nny + 1);
                else graph.addEdge(nnx * M + nny + 1, nx * M + ny + 1);
            }
        }
    }
    cout << graph.get();
