...
void solve() {
    ...
    while (!q.empty()) {
        ...
        int dist = q.top()[2];
        q.pop();
        if (visited[cur][hist] != -1) continue;
        visited[cur][hist] = dist;

        int x = arr[cur].second;
        for (int i = 0; i < K; i++) {
            if (hist & (1 << i)) continue;
            if (visited[i][hist | (1 << i)] != -1) continue;
            int nx = arr[i].second;
            if (dist + abs(x - nx) >= arr[i].first) continue;
            q.push({ i, hist | (1 << i), dist + abs(x - nx) + 1 });
        }
    }
    int ans = inf;
    for (int i = 0; i < K; i++) {
        if (visited[i][(1 << K) - 1] != -1) ans = min(ans, visited[i][(1 << K) - 1]);
    }
    if (ans == inf) cout << -1;
    else cout << ans;
