...
pair<ull,ull> tree[200004];

pair<ull, ull> mer(pair<ull, ull> a, pair<ull, ull> b) {
...
pair<ull, ull> get(int node, int start, int end, int left, int right) {
...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ull v = arr[i].first;
        ull x = arr[i].second;
        pair<ull, ull> ret1 = get(1, 1, 50001, 1, x);
        pair<ull, ull> ret2 = get(1, 1, 50001, x, 50001);
        ans += (x * ret1.first + ret2.second - ret1.second - x*ret2.first) * v;
        update(1, 1, 50001, x);
    }
    cout << ans;
