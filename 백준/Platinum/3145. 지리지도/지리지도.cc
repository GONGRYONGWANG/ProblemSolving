...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < M; j++) {
            ...
            cin >> x;
            if (x == '.') board[i][j] = -1;
            else if (x == 'x') board[i][j] = -2;
            else board[i][j] = name.size();
            if (x == '.' || x == 'x') {
                if (ret != "") {
                    name.push_back(ret);
                    ret = "";
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            for (int dir = 0; dir < 8; dir++) {
                ...
                if (E[city.size()].count(board[nx][ny]+1)) continue;
                E[city.size()].insert(board[nx][ny]+1);
            }
        }
    }

    BiMatch flow(city.size(), name.size());

    for (auto iter = E.begin(); iter != E.end(); iter++ ) {
        for (auto it = (iter->second).begin(); it != (iter->second).end(); it++) {
            flow.addEdge(iter->first, *it);
        }
    }

    flow.get();

    vector<int> trace = flow.trace();

    for (int i = 1; i <= city.size(); i++) {
        int u = i - 1;
        int v = trace[i] - 1;
        cout << city[u].first+1 << " " << city[u].second+1 << " " << name[v] << endl;
    }
