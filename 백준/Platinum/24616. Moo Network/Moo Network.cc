...
void solve(int tc) {
    ...
    for (int y = 0; y <= 10; y++) sort(q[y].begin(), q[y].end());

    vector<vector<ll>> E;

    for (int i = 0; i < N; i++) {
        auto [x, y] = arr[i];
        int idx = q[y].front().second;
        q[y].pop_front();

        ll thres = inf;
        if (!q[y].empty()) thres = q[y].front().first;
        for (int y2 = y - 1; y2 >= 0; y2--) {
            if (q[y2].empty()) continue;
            ll x2 = q[y2].front().first;
            if (x2 >= thres) continue;
            thres = x2;
            int idx2 = q[y2].front().second;
            E.push_back({ idx,idx2,(x - x2) * (x - x2) + (y - y2) * (y - y2) });
        }
        ...
        if (!q[y].empty()) {
            int idx2 = q[y].front().second;
            ll x2 = q[y].front().first;
            E.push_back({ idx,idx2,(x - x2) * (x - x2) });
        }
    }
    ...
    for (int i = 0; i < E.size(); i++) {
        int u = E[i][0]; int v = E[i][1]; ll d = E[i][2];
        u = findp(u); v = findp(v);
        if (u == v) continue;
        ans += d;
        parent[v] = u;
