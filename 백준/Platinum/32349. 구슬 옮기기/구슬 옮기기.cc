...
void solve() {
    ...
    unordered_set<pii, PiiHasher> inA;
    ...
    unordered_set<pii, PiiHasher> inAB;
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            if (x) {
                B.push_back({ i,j });
                if (inA.count({ i,j })) inAB.insert({ i,j });
            }
        }
    }
    ...
    int ans = 2 * (A.size()-inAB.size());

    BiMatch graph(A.size(), B.size());

    for (int i = 0; i < A.size(); i++) {
        if (inAB.count(A[i])) continue;
        for (int j = 0; j < B.size(); j++) {
            if (inAB.count(B[j])) continue;

            if (abs(A[i].first - B[j].first) + abs(A[i].second - B[j].second) == 1) {
