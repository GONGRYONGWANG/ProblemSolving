...
void solve() {
    ...
    int N;
    cin >> N;
    unordered_map<int, vector<pii>> r;
    ...
    for (int i = 0; i < N; i++) {
        ...
        r[x + w].push_back({ y + h,i });
        c[y + h].push_back({ x + w,i });
    }
    ...
    for (int i = 0; i < N; i++) {
        int x, y, w, h;
        x = arr[i][0]; y = arr[i][1]; w = arr[i][2]; h = arr[i][3];

        if (!r[x].empty()) {
            auto it = lower_bound(r[x].begin(), r[x].end(), make_pair(y, 0));
            while (it != r[x].end() && (it->first) <= y + h) {
                parent[findp(it->second)] = findp(i);
                it++;
            }
        }

        if (!r[x + w].empty()) {
            auto it = lower_bound(r[x + w].begin(), r[x + w].end(), make_pair(y, 0));
            while (it != r[x + w].end() && (it->first) <= y + h) {
                parent[findp(it->second)] = findp(i);
                it++;
            }
        }
        ...
        if (!c[y].empty()) {
            auto it = lower_bound(c[y].begin(), c[y].end(), make_pair(x, 0));
            while (it != c[y].end() && (it->first) <= x + w) {
                parent[findp(it->second)] = findp(i);
                it++;
            }
        }
        ...
        if (!c[y + h].empty()) {
            auto it = lower_bound(c[y + h].begin(), c[y + h].end(), make_pair(x, 0));
            while (it != c[y + h].end() && (it->first) <= x + w) {
                parent[findp(it->second)] = findp(i);
                it++;
            }
        }
