...
pii get_dia(int x) {
    ...
    for (int nx : child[x]) {
        ...
        else if (c.second > mx2) mx2 = c.second;
    }
    d = max(d, mx - depth[x] + mx2 - depth[x]);

    return diameter[x] = { d,mx };
}

void get_p_route(int x) {
    int mx = depth[x];
    int mx2 = depth[x];
    for (int nx : child[x]) {
        if (diameter[nx].second >= mx) {
            mx2 = mx;
            mx = diameter[nx].second;
        }
        else if (diameter[nx].second > mx2) mx2 = diameter[nx].second;
    }
    for (int nx : child[x]) {
        if (diameter[nx].second == mx) {
            p_route[nx] = max(p_route[x] + 1, mx2 - depth[x] + 1);
        }
        else p_route[nx] = max(p_route[x] + 1, mx - depth[x] + 1);
        get_p_route(nx);
    }
}
...
void solve() {
    ...
    for (int i = 2; i <= N; i++) {
        cout << max(diameter[1].first, diameter[i].first + p_route[i]) << endl;
    }
