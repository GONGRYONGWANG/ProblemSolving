...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
            if (j == i) continue;
            v.push_back({ arr[j].first - arr[i].first,arr[j].second - arr[i].second });
        }
        ...
        ll r = 0;
        for (int l = 0; l < N - 1; l++) {
            if (quadrant(v[l]) >= 2) break;
            while (r <= l) r += 1;
            while (r != N - 1 && cmp(v[r], { -v[l].first,-v[l].second })) r += 1;
            cnt[l] = N - 1 - r;
        }
        ...
        psum[0] = cnt[0];
        for (int l = 1; l < N - 1; l++) {
            psum[l] = psum[l - 1] + cnt[l];
        }

        for (int l = 0; l < N - 1; l++) {
            if (quadrant(v[l]) >= 2) break;
            ans -= cnt[l] * (N - 2 - cnt[l] - l) - (psum[N - 2 - cnt[l]] - psum[l]);
        }
    }
