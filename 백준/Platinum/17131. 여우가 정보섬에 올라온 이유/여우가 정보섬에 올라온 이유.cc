...
ll get(int node, int start, int end, int left, int right, vector<ll>& tree) {
    if (right < left) return 0;
    if (left>end || right<start) return 0;
    if (left <= start && right >= end) return tree[node];
    int mid = (start + end) / 2;
    return (get(node * 2, start, mid, left, right, tree) + get(node * 2 + 1, mid + 1, end, left, right, tree)) % mod;
}

void update(int node, int start, int end, int x, ll val,  vector<ll> &tree) {
    if (x>end || x<start) return;
    tree[node] += val;
    tree[node] %= mod;
    if (start == end) return;
    int mid = (start + end) / 2;
    update(node * 2, start, mid, x, val, tree);
    update(node * 2 + 1, mid + 1, end, x, val, tree);
}
...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        compress[arr[i].second] = 1;
    }
    int cnt = 0;
    for (auto iter = compress.begin(); iter != compress.end(); iter++) {
        cnt += 1;
        iter->second = cnt;
    }
    for (int i = 0; i < N; i++) {
    ...
    tree[0].resize(4 * cnt, 0);
    tree[1] = tree[2] = tree[0];

    vector<pair<int, vector<ll>>> ret;
    for (int i = 0; i < N; i++) {
        int x = arr[i].first;
        int y = arr[i].second;
        if (i != 0 && x != arr[i - 1].first) {
            while(!ret.empty()){
                auto p = ret.back();
                ret.pop_back();
                for (int j = 0; j < 3; j++) {
                    update(1, 1, cnt, p.first, p.second[j], tree[j]);
                }
            }
        }
        ret.push_back({ y, { 1, get(1,1,cnt,y + 1,cnt, tree[0]), get(1,1,cnt,1,y - 1,tree[1])} });
    }
    while (!ret.empty()) {
        auto p = ret.back();
        ret.pop_back();
