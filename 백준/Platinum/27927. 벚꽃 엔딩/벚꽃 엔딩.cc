...
void update(int x) {
    ...
    int n = x / 316;
    bck[n] = { 0,0,0 };
    for (int i = n * 316; i < (n + 1) * 316; i++) {
        if (!flower[i]) break;
        bck[n][0] += 1;
    }
    for (int i = (n + 1) * 316 - 1; i >= n * 316; i--) {
        if (!flower[i]) break;
        bck[n][2] += 1;
    }
    int tmp = 0;
...
int get() {
    ...
    for(int i=0; i<317;i++){
        ret = max(ret, bck[i][1]);
        ret = max(ret, tmp + bck[i][0]);
        if (bck[i][1] == 316) tmp += 316;
        else tmp = bck[i][2];
    }
    return ret;
}
...
void solve(int tc) {
    ...
    for (int i = 0; i < 317; i++) bck[i] = { 0,0,0 };
    ...
    for (int i = 0; i < 2 * N - 1; i++) {
        ...
        update(x);
        int ret = get();
        if (ret > ans.first) ans = { ret,0 };
        if (ret == ans.first) ans.second += arr[i + 1].first - arr[i].first;
    }
