...
void solve() {
    ...
    while (!q.empty()) {
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            int px = x - dx[dir]; int py = y - dy[dir];
            nx = x + dx[dir] * 2; ny = y + dy[dir]*2;
            while (nx >= 0 && nx < N && ny >= 0 && ny < M && px >= 0 && px < N && py >= 0 && py < M && board[px][py] != '#') {
                if (board[nx][ny] != '#' && !visited[nx][ny]) {
                    q.push({ nx,ny });
                    visited[nx][ny] = true;
                }
                px -= dx[dir]; py -= dy[dir]; nx += dx[dir]; ny += dy[dir];
            }
        }
    }
