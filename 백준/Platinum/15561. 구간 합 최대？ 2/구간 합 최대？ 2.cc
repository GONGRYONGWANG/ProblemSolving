...
vector<int> tree[4000000]; // {mn,mx, best};
int lazy[1000001];
int arr[1000001];

vector<int> mer(vector<int>& a, vector<int>& b) {
    return { min(a[0],b[0]), max(a[1],b[1]), max(max(a[2],b[2]),b[1] - a[0]) };
}

void lazyProp(int node, int start, int end) {
    if (lazy[node] == 0) return;
    tree[node] = { tree[node][0] + lazy[node], tree[node][1] + lazy[node], tree[node][2] };
    if (start != end) {
...
vector<int> get(int node, int start, int end, int left, int right) {
    ...
    int mid = (start + end) / 2;
    vector<int> a = get(node * 2, start, mid, left, right);
    vector<int> b = get(node * 2 + 1, mid + 1, end, left, right);
    return mer(a, b);
}
...
void solve() {
    ...
    while (Q--) {
        ...
        if (!C) {
            if (A == 1) {
                cout << max(get(1, 1, N, A, B)[2], get(1, 1, N, A, B)[1]) - V << endl;
            }
            else cout << get(1, 1, N, A - 1, B)[2] - V << endl;
        }
        else {
            update(1, 1, N, A, N, B * U + V - arr[A]);
            arr[A] = B * U + V;
        }
    }
