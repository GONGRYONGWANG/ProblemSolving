...
ll DP[1001][1001];
ll dp(ll cur, ll p) {
    if ((cur == N && p == 0) || (cur == 0 && p == N)) return 0;
    ...
    ll ret = INF;
    if (l != 0) ret = min(ret, dp(l - 1, r) + n * (arr[cur] - arr[l - 1]));
    if (r != N) ret = min(ret, dp(r + 1, l) + n * (arr[r + 1] - arr[cur]));
...
void solve() {
    ...
    cout << dp(lower_bound(arr.begin(), arr.end(), 0) - arr.begin(), lower_bound(arr.begin(),arr.end(),0) - arr.begin());
