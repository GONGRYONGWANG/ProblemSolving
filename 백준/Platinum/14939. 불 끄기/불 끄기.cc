...
void solve(int tc) {
    ...
    for (int hist = 0; hist < (1 << 10); hist++) {
        for (int j = 0; j < 10; j++) {
            if (hist & (1 << j)) board[0][j] = true;
        }
        ...
        for (int i = 1; i <= 10; i++) {
            for (int j = 0; j < 10; j++) {
                if (board[i - 1][j]) {
                    ...
                    if (j != 9) board[i][j + 1] = !board[i][j + 1];
                    if (i != 10) board[i + 1][j] = !board[i + 1][j];
                }
            }
        }
        ...
        for (int i = 1; i <= 10; i++) {
            for (int j = 0; j < 10; j++) {
                if (board[i][j]) b = false;
            }
        }

        if (b) ans = min(ans, ret);
    }
