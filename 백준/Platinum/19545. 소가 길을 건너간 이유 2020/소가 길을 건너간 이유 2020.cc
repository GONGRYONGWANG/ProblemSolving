...
vector<ll> A, B;
ll dist(int a, int b) {
    return L * L + (A[a] - B[b]) * (A[a] - B[b]);
}
ll dp(int a, int b, int x) {
    if (a == N && b == M) return 0;
    if (DP[a][b][x]) return DP[a][b][x];
    if (!x) {
        if (b == M) return INF;
        return DP[a][b][x] = min(dp(a, b + 1, x) + dist(a - 1, b) * (N + M - 1), dp(a, b + 1, !x) + dist(a - 1, b) * (a + b) * (N + M - a - b));

    }
    else {
        if (a == N) return INF;
        return DP[a][b][x] = min(dp(a + 1, b, x) + dist(a, b - 1) * (N + M - 1), dp(a + 1, b, !x) + dist(a, b - 1) * (a + b) * (N + M - a - b));
    }
}
