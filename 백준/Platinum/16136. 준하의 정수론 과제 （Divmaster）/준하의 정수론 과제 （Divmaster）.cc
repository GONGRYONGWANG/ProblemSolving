...
int cal[1000001];
int isnprime[1000001];
vector<int> prime;

int arr[100001];
ll bucketsum[317];
int bucketupdatecnt[317];

int N, Q;

void updatebucket(int bck) {
    for (int i = bck * 316; i < min(((bck + 1) * 316), N); i++){
        bucketsum[bck] -= arr[i];
        arr[i] = cal[arr[i]];
        bucketsum[bck] += arr[i];
    }
...
void solve() {
    for (int i = 2; i <= 1000; i++) {
        if (!isnprime[i]) prime.push_back(i);
    ...
    while (Q--) {
        ...
        if (t == 1) {
            ...
            for (int j = lbck + 1; j < rbck; j++) {
                if (bucketupdatecnt[j] == 6) continue;
                updatebucket(j);
                bucketupdatecnt[j] += 1;
            }
