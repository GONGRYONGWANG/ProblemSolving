...
void solve(int tc) {
    ...
    for (int i = 1; i <= N; i++) arr[i] = comp[arr[i]];
    ...
    auto cmp = [&](vector<int>& a, vector<int>& b) {
        if (a[0] / int(sqrt(N)) == b[0] / int(sqrt(N))) return a[1] < b[1];
        return a[0] / int(sqrt(N)) < b[0] / int(sqrt(N));
    };
    ...
    int s = 1; int e = 1;
    for (int i = 0; i < Q; i++) {
        int l = query[i][0]; int r = query[i][1] + 1;
        while (l < s) {
            s -= 1;
