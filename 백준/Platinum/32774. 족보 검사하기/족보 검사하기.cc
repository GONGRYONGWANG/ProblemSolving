...
bool ins(vector<string>& v, char c) {
    for (string s : v) {
        for (char x : s) {
            if (x == c) return true;
        }
...
bool valid(vector<string>& p1, vector<string>& p2, string s) {
    return (ins(p1, s[0]) && ins(p2, s[1])) | (ins(p1, s[1]) && ins(p2, s[0]));
}
...
void solve() {
    ...
    for (int i = 1; i <= N; i++) {
        ...
        cin >> s;
        if (s == "A") arr[i] = { "AA","AO" };
        else if (s == "B") arr[i] = { "BB","BO" };
        else if (s == "O") arr[i] = { "OO" };
        else arr[i] = { "AB" };
    }
    ...
    while (!q.empty()) {
        ...
        int p1 = parent[x].first;
        int p2 = parent[x].second;

        vector<string> ret;
        for (string s : arr[x]) {
            if (valid(arr[p1], arr[p2], s)) ret.push_back(s);
        }
