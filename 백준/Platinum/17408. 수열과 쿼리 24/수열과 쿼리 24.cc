...
void update(int node, int start, int end, int target, int val) {
    ...
    vector<int> v = { tree[node * 2].first, tree[node * 2].second, tree[node * 2 + 1].first, tree[node * 2 + 1].second };
...
pii get(int node, int start, int end, int left, int right) {
    ...
    int mid = (start + end) / 2;
    pii a = get(node * 2, start, mid, left, right);
    pii b = get(node * 2 + 1, mid + 1, end, left, right);
