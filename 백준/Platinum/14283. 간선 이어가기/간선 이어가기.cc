...
struct MF { // Dinic
    ...
    bool other_component(int u, int v , int src){
...
void solve() {
    ...
    for (int i = 0; i < M; i++) {
        MF flow(N + 3);
        int src = N + 1; int sink = N + 2;
        for (int j = 0; j < M; j++) {
            if (i == j) continue;
            int u, v, w;
            u = E[j][0]; v = E[j][1]; w = E[j][2];
            flow.addEdge(u, v, w, 0);
        }
        flow.addEdge(src, s, INF); flow.addEdge(t, sink, INF);
        int ret = total - flow.get(src, sink);
        if (flow.other_component(E[i][0], E[i][1], src)) ans = max(ans, ret);
    }
