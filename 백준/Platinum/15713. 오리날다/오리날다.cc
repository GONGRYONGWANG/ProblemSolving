...
void solve() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        arr[i].second = min(arr[i].second, S - arr[i].first);
    }
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (!m.count(y + h)) m[y + h] = INF;
        m[y + h] = min((it->first) - y + (it->second) + h, m[y + h]);
        it = m.find(y + h);
        auto rit = it;
        rit++;
        if (rit != m.end()) {
            if ((it->second) + (it->first) >= (rit->second) + (rit->first)) {
                m.erase(it);
                continue;
            }
        }
        ...
        while (1) {
            auto lit = it;
            lit--;
            if ((lit->second) + (lit->first) >= (it->second) + (it->first)) {
                m.erase(lit);
            }
            else break;
        }
