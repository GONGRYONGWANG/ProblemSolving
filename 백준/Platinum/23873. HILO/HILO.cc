...
int get(int node, int start, int end, int left, int right, int mx, int mn) {
    if (left > end || right < start) return 0;
    if (upper_bound(tree[node].begin(), tree[node].end(), mn) == lower_bound(tree[node].begin(), tree[node].end(), mx)) return 0;
    if (start == end) return tree[node][0];
    int mid = (start + end) / 2;
    int l = get(node * 2, start, mid, left, right, mx, mn);
    if (l != 0) return l;
    return get(node * 2 + 1, mid + 1, end, left, right, mx, mn);
}
...
void solve(int tc) {
    ...
    vector<int> H, L;
    for (int i = 1; i <= N; i++) {
        if (H.empty() || arr[H.back()] > arr[i]) H.push_back(i);
    }
    cout << ret << endl;

    for (int x = 1; x <= N; x++) {
        if (!H.empty() && H.back() == pos[x]) {
            if (upper_bound(L.begin(), L.end(), pos[x]) != L.end()) {
                if (H.size() == 1) ret -= 1;
                else if (upper_bound(L.begin(), L.end(), pos[x]) != upper_bound(L.begin(), L.end(), H[H.size() - 2])) ret -= 1;
            }
            H.pop_back();
            while (1) {
                int g = 0;
                if (pos[x] != N) {
                    if (H.empty()) g = get(1, 1, N, pos[x] + 1, N, N + 1, x);
                    else g = get(1, 1, N, pos[x] + 1, N, arr[H.back()], x);
                }
                if (g == 0) break;
                if (!L.empty() && upper_bound(L.begin(), L.end(), pos[g]) != L.end()) {
                    if (H.empty() || upper_bound(L.begin(), L.end(), pos[g]) != upper_bound(L.begin(), L.end(), H.back())) ret += 1;
                }
                H.push_back(pos[g]);
            }
        }

        while (!L.empty() && pos[x] < L.back()) {
            if (upper_bound(H.begin(), H.end(), L.back()) != H.begin()) {
                if (L.size() == 1) ret -= 1;
                else if (upper_bound(H.begin(), H.end(), L.back()) != upper_bound(H.begin(), H.end(), L[L.size() - 2])) ret -= 1;
            }
            L.pop_back();
        }
        if (upper_bound(H.begin(), H.end(), pos[x]) != H.begin()) {
            if (L.empty()) ret += 1;
            else if (upper_bound(H.begin(), H.end(), L.back()) != upper_bound(H.begin(), H.end(), pos[x])) ret += 1;
        }
        L.push_back(pos[x]);
