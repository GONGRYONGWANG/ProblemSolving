...
void solve() {
    ...
    while (!q.empty()) {
        ...
        int d = q.top()[2];
        q.pop();
        if (visited[cur] <= h || dist[cur][h] != -1) continue;
        visited[cur] = h;
        dist[cur][h] = d;
        for (pii& e : E[cur]) {
            if (visited[e.first] <= h + 1) continue;
            if (dist[e.first][h + 1] != -1) continue;
            q.push({ e.first,h + 1,e.second + d });
        }
    }
    int ret = inf;
    for (int h = 0; h < N; h++) {
        if (dist[b][h] == -1) continue;
        ret = min(ret, dist[b][h]);
    }
    cout << ret << endl;
    for (int i = 1; i <= K; i++) {
        int x;
