...
void update(int node, int start, int end, int a, ll v) {
    ...
    int mid = (start + end) / 2;
    update(node * 2, start, mid, a, v);
    update(node * 2 + 1, mid + 1, end, a, v);
    tree[node] = tree[node * 2] + tree[node * 2 + 1];
}
...
void solve() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        if (t == 1) {
            int a, v;
            cin >> a >> v;
            update_q.push_back({ a,v });
        }
        else {
            int k, l, r;
            cin >> k >> l >> r;
            get_q.push_back({ k,l,r,i });
        }
    }

    sort(get_q.begin(), get_q.end());
    vector<pll> ans;

    int K = 0;

    for (vector<int>& q : get_q) {
        int k, l, r, idx;
        k = q[0]; l = q[1]; r = q[2]; idx = q[3];
        while (K != k) {
            update(1, 1, N, update_q[K].first, update_q[K].second);
            K += 1;
        }
        ans.push_back({ idx, get(1,1,N,l,r) });
    }
