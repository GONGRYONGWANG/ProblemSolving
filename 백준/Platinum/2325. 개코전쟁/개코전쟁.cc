...
void solve() {
    ...
    while (node != 1) {
        ...
        discon.first = node;
        for (pii& e : E[node]) {
            if (dist[e.first] + e.second == dist[node]) {
                discon.second = e.first;
                node = e.first;
                break;
            }
        }
        ...
        while (!q.empty()) {
            ...
            int d = q.top().second;
            q.pop();
            if (distance[x]) continue;
            distance[x] = d;
            for (pii& e : E[x]) {
                if (discon == make_pair( e.first, x )) continue;
                if (discon == make_pair(x, e.first)) continue;
                if (distance[e.first]) continue;
                q.push({ e.first,e.second + d });
            }
        }
