...
void solve() {
    ...
    for (int i = -100000; i <= 100000; i++) {
        ...
        ret.insert((*m[i].rbegin()) - (*m[i].begin()));
    }
    ...
    while (M--) {
        ...
        ret.erase(ret.find((*m[psum[p]].rbegin()) - (*m[psum[p]].begin())));
        m[psum[p]].erase(p);
        if (m[psum[p]].empty()) m.erase(psum[p]);
        else ret.insert((*m[psum[p]].rbegin()) - (*m[psum[p]].begin()));

        if (arr[p]) psum[p] -= 2;
        else psum[p] += 2;

        if (m.count(psum[p])) ret.erase(ret.find((*m[psum[p]].rbegin()) - (*m[psum[p]].begin())));
        m[psum[p]].insert(p);
        ret.insert((*m[psum[p]].rbegin()) - (*m[psum[p]].begin()));
